...
class _SeguimientoScreenState extends State<SeguimientoScreen> {
  ...
  Map<String, dynamic> _payload() => {
    'fechaRegistro': _fecha.toIso8601String().substring(0, 10),
    'tipoSeguimiento': modulo.id,
    'loteId': widget.lote.id,
    for (final e in _c.entries) e.key: e.value.text,
    'itemsHembras': _itemsH.map((i) => i.toJson()).toList(),
    'itemsMachos': _itemsM.map((i) => i.toJson()).toList(),
    'itemsGenerales': _itemsG.map((i) => i.toJson()).toList(),
  };
  ...
  List<Widget> _engorde() => [
    _general(),
    _itemsSec('itemsG', 'Alimento del lote', _itemsG, acento: AppColors.orange600,
      nota: 'Solo se listan ítems con existencia disponible en inventario.'),
    _mortalidadSec(),
  ...
  List<Widget> _produccion() => [
    ...
    AppSection(
      ...
      filled: lleno(['mortalidadH', 'selH']),
      children: [
        Row(children: [
          Expanded(child: AppField(label: 'Mortalidad', required: true, controller: ctl('mortalidadH'),
    ...
    AppSection(
      ...
      filled: lleno(['mortalidadM', 'selM']),
      children: [
        Row(children: [
          Expanded(child: AppField(label: 'Mortalidad', required: true, controller: ctl('mortalidadM'),
            keyboardType: TextInputType.number)),
          const SizedBox(width: AppSpacing.s2),
  ...
  List<Widget> _reproductora() => [
    AppSection(
      ...
      children: [
        _fechaField(),
        AppField(label: 'Lote', controller: TextEditingController(text: widget.lote.nombre), readOnly: true),
        AppField(label: 'Reproductora', required: true, controller: ctl('reproductoraId'),
          placeholder: 'Selecciona la reproductora'),
        AppField(label: 'Ciclo', controller: ctl('ciclo'), placeholder: 'Normal'),
        AppField(label: 'Observaciones', controller: ctl('observaciones'),
          placeholder: 'Novedades del día…', maxLines: 3),
      ],
    ...
    AppSection(
      title: 'Hembras ♀ — ítems y bajas',
      icon: Icons.female_rounded,
      expanded: abierto('hembras'), onToggle: () => toggle('hembras'),
      filled: lleno(['mortalidadH']) || _itemsH.isNotEmpty,
      children: [
        _ItemsEditor(items: _itemsH, acento: AppColors.hembra, onChanged: () => setState(() {})),
        Row(children: [
          Expanded(child: AppField(label: 'Mortalidad', required: true, controller: ctl('mortalidadH'),
    ...
    AppSection(
      title: 'Machos ♂ — ítems y bajas',
      icon: Icons.male_rounded,
      expanded: abierto('machos'), onToggle: () => toggle('machos'),
      filled: lleno(['mortalidadM']) || _itemsM.isNotEmpty,
      children: [
        _ItemsEditor(items: _itemsM, acento: AppColors.macho, onChanged: () => setState(() {})),
        Row(children: [
          Expanded(child: AppField(label: 'Mortalidad', required: true, controller: ctl('mortalidadM'),
    ...
    AppSection(
      ...
      children: [
        Row(children: [
          Expanded(child: AppField(label: 'Peso inicial', suffix: 'kg', controller: ctl('pesoInicial'),
            keyboardType: const TextInputType.numberWithOptions(decimal: true))),
          const SizedBox(width: AppSpacing.s2),
          Expanded(child: AppField(label: 'Peso final', suffix: 'kg', controller: ctl('pesoFinal'),
            keyboardType: const TextInputType.numberWithOptions(decimal: true))),
        ]),

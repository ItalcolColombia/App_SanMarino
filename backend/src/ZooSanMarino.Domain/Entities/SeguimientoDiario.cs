...
public class SeguimientoDiario
{
    ...
    /// caminos que separan (<c>Validado = !separa</c>). Quien crea la fila sin decir nada —traslados,
    /// arrastre de huevos, movimientos, cargas— no separa nada, así que no hay nada que validar. Con
    /// el valor inicial en <c>false</c> esto falló cuatro veces (los Crud, los traslados, el cruce de
    /// reproductora y la rama Colombia de Producción): cada creador tenía que acordarse, y el que no
    /// lo hacía dejaba filas «pendientes» que a las 24 h bloqueaban el alta de días nuevos del lote
    /// sin tener nada que validar.
    /// </para>
    /// <para>

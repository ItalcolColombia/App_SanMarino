...
public class GuiaGeneticaEngordeService : IGuiaGeneticaEngordeService
{
    ...
    static GuiaGeneticaEngordeService()
    {
        ExcelPackage.License.SetNonCommercialPersonal("ZooSanMarino");
    }

    public GuiaGeneticaEngordeService(
    ...
    private async Task<GuiaGeneticaEngordeHeader> GetOrCreateHeaderAsync(
        int companyId,
        int paisId,
        string raza,
        int anioGuia,
        ...
        CancellationToken ct)
    {
        ...
        var header = await _ctx.GuiaGeneticaEngordeHeader
            .FirstOrDefaultAsync(
        ...
        if (header == null)
        {
            header = new GuiaGeneticaEngordeHeader
            {
                PaisId = paisId,
                Raza = razaTrim,
                AnioGuia = anioGuia,
            ...
            _ctx.GuiaGeneticaEngordeHeader.Add(header);
    ...
    private async Task ReplaceDetallesSexoAsync(
        ...
        int companyId,
        string sexo,
        IReadOnlyList<GuiaGeneticaEngordeDetalle> nuevos,
        CancellationToken ct)
    {
        var existentes = await _ctx.GuiaGeneticaEngordeDetalle
            .Where(d => d.GuiaGeneticaEngordeHeaderId == headerId && d.Sexo == sexo)
            .ToListAsync(ct);

        if (existentes.Count > 0)
            _ctx.GuiaGeneticaEngordeDetalle.RemoveRange(existentes);

        foreach (var d in nuevos)
        {
            d.GuiaGeneticaEngordeHeaderId = headerId;
            d.Sexo = sexo;
        ...
        _ctx.GuiaGeneticaEngordeDetalle.AddRange(nuevos);
    }

    public async Task<GuiaGeneticaEngordeFiltersDto> GetFiltersAsync(CancellationToken ct = default)
    {
        var cid = await GetEffectiveCompanyIdAsync(ct);
        var pid = GetEffectivePaisId();
        var razas = await _ctx.GuiaGeneticaEngordeHeader.AsNoTracking()
        ...
        var anos = await _ctx.GuiaGeneticaEngordeHeader.AsNoTracking()
            .Where(h => h.CompanyId == cid && h.PaisId == pid && h.DeletedAt == null)
            .Select(h => h.AnioGuia)
            .Distinct()
            .OrderBy(a => a)
        ...
        return new GuiaGeneticaEngordeFiltersDto(razas, anos);
    }
    ...
    public async Task<IEnumerable<int>> GetAnosPorRazaAsync(string raza, CancellationToken ct = default)
    {
        ...
        return await _ctx.GuiaGeneticaEngordeHeader.AsNoTracking()
            .Where(h =>
                h.CompanyId == cid &&
                h.PaisId == pid &&
                h.DeletedAt == null &&
    ...
    public async Task<IEnumerable<string>> GetSexosCreadosAsync(string raza, int anioGuia, CancellationToken ct = default)
    {
        ...
        var headerId = await _ctx.GuiaGeneticaEngordeHeader.AsNoTracking()
            .Where(h =>
                h.CompanyId == cid &&
                h.PaisId == pid &&
                h.DeletedAt == null &&
        ...
        var sexos = await _ctx.GuiaGeneticaEngordeDetalle.AsNoTracking()
            .Where(d => d.GuiaGeneticaEngordeHeaderId == headerId && d.DeletedAt == null)
            .Select(d => d.Sexo)
            .Distinct()
            .ToListAsync(ct);
    ...
    public async Task<IEnumerable<GuiaGeneticaEngordeDetalleDto>> GetDatosAsync(string raza, int anioGuia, string sexo, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(raza) || anioGuia <= 0 || string.IsNullOrWhiteSpace(sexo))
            return Array.Empty<GuiaGeneticaEngordeDetalleDto>();
        ...
        return await _ctx.GuiaGeneticaEngordeDetalle.AsNoTracking()
            .Where(d => d.GuiaGeneticaEngordeHeaderId == headerId && d.Sexo == sexoN && d.DeletedAt == null)
            .OrderBy(d => d.Dia)
            .Select(d => new GuiaGeneticaEngordeDetalleDto(
                d.Sexo,
                d.Dia,
                d.PesoCorporalG,
                d.GananciaDiariaG,
    ...
    public async Task<GuiaGeneticaEngordeImportResultDto> ImportExcelAsync(
        IFormFile file,
        string raza,
        int anioGuia,
        string estado,
        CancellationToken ct = default)
    {
        ...
        if (!validation.IsValid)
            return new GuiaGeneticaEngordeImportResultDto(false, 0, 0, 0, validation.Errors);
        ...
        if (string.IsNullOrWhiteSpace(raza) || anioGuia <= 1900 || anioGuia > 2100)
        {
            ...
            return new GuiaGeneticaEngordeImportResultDto(false, 0, 0, 0, errors);
        }
        ...
        await using (var tx = await _ctx.Database.BeginTransactionAsync(ct))
        {
            try
            {
                ...
                foreach (var sexo in SexosValidos)
                {
                    ...
                    var nuevos = new List<GuiaGeneticaEngordeDetalle>();
                    ...
                    for (var row = firstDataRow; row <= lastRow; row++)
                    {
                        ...
                        nuevos.Add(new GuiaGeneticaEngordeDetalle
                        {
                            Dia = dia,
                            PesoCorporalG = peso,
                ...
                if (!processedAnySheet)
                {
                    ...
                    return new GuiaGeneticaEngordeImportResultDto(false, 0, 0, 1, errors);
                }

                if (errors.Count > 0)
                {
                    await tx.RollbackAsync(ct);
                    return new GuiaGeneticaEngordeImportResultDto(false, filasProcesadas, 0, errorFilas, errors);
                }
            ...
            catch (Exception ex)
            {
                ...
                return new GuiaGeneticaEngordeImportResultDto(false, filasProcesadas, detallesInsertados, errorFilas + 1, errors);
            }
        }

        var ok = errors.Count == 0;
        return new GuiaGeneticaEngordeImportResultDto(ok, filasProcesadas, detallesInsertados, errorFilas, errors);
    }

    public async Task<GuiaGeneticaEngordeHeaderDto> UpsertManualAsync(GuiaGeneticaEngordeManualRequestDto request, CancellationToken ct = default)
    {
        ...
        return new GuiaGeneticaEngordeHeaderDto(header.Id, header.Raza, header.AnioGuia, header.Estado);
    }

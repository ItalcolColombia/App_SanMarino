...
public partial class ProduccionService : IProduccionService
{
    ...
    /// <summary>
    /// Valida el desglose de huevos por ítems del request: catálogo activo de la empresa de la granja,
    /// lista blanca del lote (<c>lote_huevo_items</c>) y vigencia de primera postura. Vive en
    /// <see cref="HuevoItemsLoteValidacion"/>, compartida con el seguimiento diario de LEVANTE para que
    /// las dos pantallas acepten exactamente el mismo conjunto. Lanza
    /// <see cref="InvalidOperationException"/> (el controller la traduce a 400).
    /// </summary>
    private Task<List<HuevoItemSeguimientoDto>> ValidarHuevoItemsAsync(
        int loteId, List<HuevoItemSeguimientoDto> huevoItems, DateTime fechaRegistro) =>
        HuevoItemsLoteValidacion.ValidarAsync(_context, loteId, huevoItems, fechaRegistro);

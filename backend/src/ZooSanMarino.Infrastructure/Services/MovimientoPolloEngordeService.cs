...
public class MovimientoPolloEngordeService : IMovimientoPolloEngordeService
{
    ...
    public async Task<AvesDisponiblesLotesResponse> GetAvesDisponiblesLotesAsync(AvesDisponiblesLotesRequest request)
    {
        ...
        var pendientes = await _ctx.MovimientoPolloEngorde
            ...
            .Select(m => new { Id = m.LoteAveEngordeOrigenId!.Value, m.CantidadHembras, m.CantidadMachos, m.CantidadMixtas })
            .ToListAsync();
    ...
    public async Task<AuditoriaVentasEngordeResponse> AuditarVentasEngordeAsync(AuditoriaVentasEngordeRequest request)
    {
        ...
        try
        {
            foreach (var id in ids)
            {
                ...
                var maxH = Math.Max(0, encH - lote.MortCajaH - seg.MortH - seg.SelH - seg.ErrH - asg.H);
                var maxM = Math.Max(0, encM - lote.MortCajaM - seg.MortM - seg.SelM - seg.ErrM - asg.M);
                ...
                var excesoH = Math.Max(0, totalVentasH - maxH);
                var excesoM = Math.Max(0, totalVentasM - maxM);
                var excesoX = Math.Max(0, totalVentasX - maxX);
                ...
                lotesOut.Add(new AuditoriaVentasLoteDetalle(
                    ...
                    Math.Max(0, totalVentasH - maxH),
                    Math.Max(0, totalVentasM - maxM),
                    Math.Max(0, totalVentasX - maxX),
                    autoCorregible,
                    estado
                ));
            }

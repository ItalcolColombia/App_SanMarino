// src/ZooSanMarino.Infrastructure/Persistence/Configurations/InventarioGestionStockConfiguration.cs
...
public class InventarioGestionStockConfiguration : IEntityTypeConfiguration<InventarioGestionStock>
{
    public void Configure(EntityTypeBuilder<InventarioGestionStock> e)
    {
        e.ToTable("inventario_gestion_stock", "public");
        ...
        e.HasIndex(x => new { x.FarmId, x.ItemInventarioEcuadorId, x.NucleoId, x.GalponId })
            .HasDatabaseName("ix_inventario_gestion_stock_farm_item_nucleo_galpon");
        e.HasIndex(x => x.CompanyId).HasDatabaseName("ix_inventario_gestion_stock_company_id");

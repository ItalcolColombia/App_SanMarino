...
{
    /// <summary>
    /// Confirmación por registro en seguimiento_diario_lote_reproductora_aves_engorde:
    ///   - Columnas: confirmado (bool), confirmado_at (timestamptz), confirmado_por (varchar).
    ///   - fn_cruce_reproductora_a_engorde: el cruce hacia el seguimiento pollo engorde ahora SOLO
    ///     cuenta registros CONFIRMADOS (s.confirmado = true en el JOIN y en el acumulado de aves vivas).
    ///     Efecto: un día sincroniza solo cuando TODOS los lotes reproductora confirmaron esa edad.
    ///   - Backfill: filas existentes → confirmado = true (preserva el cruce histórico; el gate aplica a lo nuevo).
    /// Idempotente (ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE). El trigger y el índice no cambian.
    /// </summary>
    public partial class AddConfirmacionSeguimientoReproductoraEngorde : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // 1) Columnas (confirmado nullable primero, para distinguir filas existentes en el backfill).
            migrationBuilder.Sql(@"
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde
    ADD COLUMN IF NOT EXISTS confirmado boolean;
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde
    ADD COLUMN IF NOT EXISTS confirmado_at timestamptz;
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde
    ADD COLUMN IF NOT EXISTS confirmado_por varchar(64);
");

            // 2) Función de cruce gateada por confirmado (mismo cálculo; solo se agregan los filtros s/p.confirmado).
            migrationBuilder.Sql(FN_GATED);

            // 3) Backfill: filas existentes → confirmado = true. Dispara el trigger (con la fn nueva) → regenera
            //    el cruce idéntico al actual (todas confirmadas). Los AFTER ROW triggers ven todas las filas ya en true.
            migrationBuilder.Sql(@"
UPDATE seguimiento_diario_lote_reproductora_aves_engorde
   SET confirmado = true
 WHERE confirmado IS NULL;
");

            // 4) Default + NOT NULL (nuevas filas nacen sin confirmar).
            migrationBuilder.Sql(@"
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde
...
");
        ...
        {
            // Revertir la función a la versión previa (sin gate de confirmado) ANTES de soltar la columna.
            migrationBuilder.Sql(FN_UNGATED);

            migrationBuilder.Sql(@"
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde DROP COLUMN IF EXISTS confirmado;
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde DROP COLUMN IF EXISTS confirmado_at;
ALTER TABLE seguimiento_diario_lote_reproductora_aves_engorde DROP COLUMN IF EXISTS confirmado_por;
");
        }

        // ── Función CON gate de confirmación (nueva) ────────────────────────────────────────────────
        private const string FN_GATED = @"
...
";
        ...
        // ── Función SIN gate (versión previa: UpdateFnCruceReproductoraEngordeAgua) para el Down ─────
        private const string FN_UNGATED = @"
CREATE OR REPLACE FUNCTION fn_cruce_reproductora_a_engorde(p_lote_ave_engorde_id int)
RETURNS void
LANGUAGE plpgsql
AS $$

...
{
    /// <summary>
    /// El saldo de aves de LEVANTE no descontaba las VENTAS, así que los reportes sobrestimaban
    /// cualquier lote que hubiera vendido aves durante la crianza.
    ///
    /// La fila diaria de levante solo guardaba `venta_aves_cantidad`, un TOTAL sin distinguir
    /// sexo, mientras el saldo se lleva POR SEXO: sin el split ninguna fórmula que reconstruya el
    /// saldo desde esas filas puede descontar la venta. Producción ya tenía resuelto esto con
    /// `venta_aves_hembras/machos` (migración VentaAvesEnFilaDiariaProduccion); acá se replica el
    /// mismo patrón en levante y las dos fns pasan a restarlas.
    ///
    /// Era además una violación de «una sola fórmula por número»: el camino C#
    /// (ReporteTecnicoService sobre SaldoAvesLevanteCalculos) SÍ descuenta la venta y coincide con
    /// el informe, mientras las dos fns SQL no. Para S-369B las fns daban 1.281 machos donde el
    /// maestro y el informe dicen 991 — los 290 de dos ventas de machos durante el levante.
    ///
    /// Las filas de PURO traslado posteriores a la semana 25, que la ventana descarta, ahora
    /// exigen además que no haya venta: una fila con venta es una fila con dato y no debe caer
    /// por esa exclusión ni contarse como base.
    ///
    /// Backfill idempotente desde movimiento_aves (el dueño del número), que solo toca filas en 0.
    ///
    /// Espejos .sql: backend/sql/fn_resumen_semanal_ra_pesadas_levante.sql y
    /// fn_reporte_semanal_levante_extras.sql (idénticos).
    /// Idempotente (ADD COLUMN IF NOT EXISTS + DROP FUNCTION IF EXISTS + CREATE OR REPLACE).
    ///
    /// Designer CLONADO y ModelSnapshot intacto, igual que hizo su hermana de producción. Acá sí
    /// se agregan dos propiedades a la entidad, así que el snapshot queda un paso atrás del
    /// modelo; se hace igual a propósito porque regenerarlo con `migrations add` arrastraría los
    /// cambios de entidad EN VUELO de otra sesión que trabaja el mismo repo. El DDL idempotente
    /// deja el desfase sin consecuencias: cuando alguien regenere el snapshot, el ADD COLUMN
    /// vuelve a ser no-op.
    /// </summary>
    public partial class VentaAvesEnFilaDiariaLevante : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"ALTER TABLE public.seguimiento_diario_levante
    ...
    ADD COLUMN IF NOT EXISTS venta_aves_machos  integer NOT NULL DEFAULT 0;");
            ...
            migrationBuilder.Sql(@"-- Backfill desde movimiento_aves, que es el DUEÑO del número. Idempotente: solo toca filas
-- que siguen en 0, así que re-correr la migración no vuelve a sumar.
-- OJO con dos particularidades de esta tabla frente a su hermana de producción:
--   * `lote_id` es VARCHAR (no integer) ⇒ hace falta el cast.
--   * no tiene `deleted_at` ⇒ no hay filtro de borrado lógico que aplicar.
WITH ventas AS (
...
UPDATE public.seguimiento_diario_levante s
  ...
  FROM ventas v
 WHERE s.lote_id = v.lote_id::text
   AND s.tipo_seguimiento = 'levante'
   AND (s.fecha AT TIME ZONE 'UTC')::date = v.dia
...
$$;");
    ...
    DROP COLUMN IF EXISTS venta_aves_machos;");
            // Las fns no se revierten: hacerlo repondría el saldo que ignora las ventas.
        }
    }
}

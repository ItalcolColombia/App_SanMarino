...
{
    /// <summary>
    /// R1 (campos vacíos): fn_indicadores_pollo_engorde devuelve NULL en los 6 campos derivados de
    /// merma (merma_unidades, merma_kilos, merma_porcentaje, ajuste_aves, porcentaje_ajuste,
    /// total_kilos_despachados_cliente) cuando Costos NO registró merma en lote_ave_engorde
    /// (merma_unidades y merma_kilos ambos NULL) ⇒ el reporte de liquidación los muestra vacíos.
    /// Con merma registrada la aritmética es idéntica a la versión previa (mismos ROUND(...,6)).
    /// CREATE OR REPLACE idempotente; espejo exacto de backend/sql/fn_indicadores_pollo_engorde.sql.
    /// </summary>
    public partial class UpdateFnIndicadoresPolloEngordeMermaNull : Migration
    {
        ...
        {
            // Restaura el cuerpo previo (versión 20260601160325_FixFnIndicadoresPolloEngordeRoundOverflow).
            migrationBuilder.Sql(FN_SQL_PREVIA, suppressTransaction: true);
        }
...
";
        ...
        private const string FN_SQL_PREVIA = @"-- ============================================================================
-- fn_indicadores_pollo_engorde (Parte A) — Liquidación Técnica Pollo Engorde EC
-- ----------------------------------------------------------------------------
-- Consolida en UNA consulta todo el cálculo que el servicio C#
-- (CalcularIndicadorLoteAveEngordeAsync) hacía con 8-10 queries N+1 por lote.

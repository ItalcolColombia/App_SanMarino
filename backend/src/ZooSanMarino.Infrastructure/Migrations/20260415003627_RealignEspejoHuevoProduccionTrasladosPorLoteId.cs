...
/// <summary>
/// Vuelve a alinear espejo_huevo_produccion con la misma fórmula que la migración anterior y que
/// EspejoHuevoProduccionSyncService: histórico = suma(produccion_diaria), dinámico = histórico − traslados Completado.
///
/// La migración anterior solo podía atribuir movimientos a LPP cuando había lote_postura_produccion_id o lote_id 'LPP-n'.
/// Esta versión también enlaza traslados cuyo lote_id es el id numérico del lote unificado (tabla lotes), buscando
/// lote_postura_produccion por company_id + lote_id, para que esos descuentos resten en los huevo_*_dinamico.
/// Idempotente: UPSERT por lote_postura_produccion_id.
/// </summary>
public partial class RealignEspejoHuevoProduccionTrasladosPorLoteId : Migration
{
    /// <inheritdoc />
...
mov_raw AS (
  SELECT
    COALESCE(
      ...
      CASE
        ...
        ELSE NULL
      END,
      lpmap.lote_postura_produccion_id
    ) AS lpp_id,
    t.cantidad_limpio,
    t.cantidad_tratado,
    t.cantidad_sucio,
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE lpp.deleted_at IS NULL
      AND lpp.company_id = t.company_id
      AND t.lote_postura_produccion_id IS NULL
      AND (t.lote_id IS NULL OR t.lote_id !~ '^LPP-[0-9]+$')
      AND trim(t.lote_id) ~ '^[0-9]+$'
      AND lpp.lote_id IS NOT NULL
      AND lpp.lote_id = trim(t.lote_id)::int
    ORDER BY lpp.lote_postura_produccion_id DESC
    LIMIT 1
  ) lpmap ON TRUE
...
");
    ...
    {
        // Datos; no revertir automáticamente.
    }
}

...
{
    /// <inheritdoc />
    public partial class AddFnIndicadoresPolloEngorde : Migration
    {
        // Parte A (performance): consolida el cálculo del indicador (antes 8-10 queries N+1 por
        // lote en C#) en una función SQL. Incluye el fix R3.1 (peso individual) y los campos R1/R2.
        /// <inheritdoc />
        ...
        {
            migrationBuilder.Sql("DROP FUNCTION IF EXISTS public.fn_indicadores_pollo_engorde(INT, NUMERIC, NUMERIC);", suppressTransaction: true);
        }
...
SELECT
    ...
    d2.superv_pct,
    d2.consumo_total,
    CASE WHEN d2.aves_sacrificadas > 0 THEN d2.consumo_total / d2.aves_sacrificadas * 1000 ELSE 0 END,
    d2.kg_carne,
    d2.peso_promedio,
    d2.conversion,
    CASE WHEN d2.conversion <= 0 THEN 0
         ELSE d2.conversion + ((p_peso_ajuste - d2.peso_promedio) / p_divisor_ajuste) END,
    p_peso_ajuste,
    p_divisor_ajuste,
    d2.edad,
    d2.metros_cuadrados,
    CASE WHEN d2.metros_cuadrados > 0 THEN d2.aves_sacrificadas::NUMERIC / d2.metros_cuadrados ELSE 0 END,
    CASE WHEN d2.metros_cuadrados > 0 THEN d2.kg_carne / d2.metros_cuadrados ELSE 0 END,
    CASE WHEN d2.conversion > 0 THEN (d2.peso_promedio / d2.conversion) * 100 ELSE 0 END,
    CASE WHEN d2.conversion > 0 AND d2.edad > 0
         THEN ((d2.peso_promedio * d2.superv_pct) / (d2.edad * d2.conversion)) * 100 ELSE 0 END,
    CASE WHEN d2.conversion > 0 THEN (d2.peso_promedio / d2.conversion) / d2.conversion * 100 ELSE 0 END,
    CASE WHEN d2.edad > 0 THEN (d2.peso_promedio / d2.edad) * 1000 ELSE 0 END,
    d2.fecha_encaset,
    ...
    d2.merma_unidades,
    d2.merma_kilos,
    CASE WHEN d2.aves_sacrificadas > 0 THEN d2.merma_unidades::NUMERIC / d2.aves_sacrificadas * 100 ELSE 0 END,
    (d2.aves_encasetadas - d2.aves_sacrificadas - d2.mortalidad - d2.merma_unidades)::INT,
    CASE WHEN d2.aves_encasetadas > 0
         THEN (d2.aves_encasetadas - d2.aves_sacrificadas - d2.mortalidad - d2.merma_unidades)::NUMERIC / d2.aves_encasetadas * 100 ELSE 0 END,
    d2.kg_carne,
    ...
    d2.liquidado_at,
    CASE WHEN d2.aves_encasetadas > 0 THEN d2.aves_sacrificadas::NUMERIC / d2.aves_encasetadas ELSE 0 END
FROM d2;
$$;
";
    }

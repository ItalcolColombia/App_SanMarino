...
{
    /// <summary>
    /// A5 (2ª parte): <c>fn_seguimiento_diario_produccion</c> deja de contar los seguimientos
    /// marcados como borrados.
    ///
    /// <para>
    /// <b>La bomba que desarma.</b> <c>seguimiento_diario_produccion</c> tiene columna
    /// <c>deleted_at</c>, la entidad hereda la propiedad de <c>AuditableEntity</c> y está mapeada en
    /// <c>SeguimientoProduccionConfiguration</c>. Pero <b>nadie la filtra</b>: no hay un solo
    /// <c>HasQueryFilter</c> en todo el backend, y esta función —la fórmula canónica del saldo diario
    /// de producción— filtraba el <c>deleted_at</c> de <c>lotes</c>, <c>lote_postura_produccion</c>,
    /// <c>lote_postura_levante</c> y <c>movimiento_aves</c>, pero no el de la tabla de la que saca
    /// los seguimientos. La infraestructura de soft delete está a medio poner: el día que algo
    /// empiece a marcar filas, seguirían contando en el saldo de aves y de huevos sin que nada avise.
    /// </para>
    ///
    /// <para>
    /// <b>Por qué es seguro aplicarlo ahora.</b> Hoy los 17 borrados de las tablas operativas son
    /// <c>DELETE</c> físico y <b>nadie escribe esa columna</b>: hay <b>0 filas</b> con
    /// <c>deleted_at</c> no nulo. O sea que el cambio es un <b>no-op verificable</b> sobre los datos
    /// existentes, y el gate de paridad lo confirma fila a fila. Es justamente el momento barato de
    /// hacerlo: después de que exista la primera fila borrada, el mismo cambio movería números.
    /// </para>
    ///
    /// <para>
    /// <b>Lo que este item NO hace.</b> No implementa soft delete. Se evaluó y se descartó con
    /// medición: los tombstones de <c>sync_tombstones</c> (migración <c>20260810031057</c>) ya cubren
    /// el requisito que el plan de la PWA invocaba para A5 —que un cliente que sincroniza por cursor
    /// se entere de los borrados—, y sin un solo <c>HasQueryFilter</c> en el proyecto, convertir los
    /// borrados en lógicos obligaría a revisar cada consulta de esas entidades a mano. Riesgo alto
    /// para cubrir algo que ya está cubierto. Detalle en
    /// <c>fase_de_desarrollo/pwa_f0_scoping_produccion_y_softdelete_plan.md</c>.
    /// </para>
    ///
    /// <para>
    /// Se conserva la <b>misma firma de 2 argumentos</b>: agregar un parámetro con <c>DEFAULT</c>
    /// crearía una <b>sobrecarga</b>, no un reemplazo, y las llamadas existentes quedarían ambiguas
    /// (lección de <c>20260810035730</c>). El espejo <c>backend/sql/fn_seguimiento_diario_produccion.sql</c>
    /// se actualiza en el mismo commit: un <c>.sql</c> cambiado sin migración queda muerto, y un
    /// espejo desincronizado ya costó tiempo antes.
    /// </para>
    /// </summary>
    public partial class FnSeguimientoProduccionExcluyeBorrados : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder) =>
            migrationBuilder.Sql(@"CREATE OR REPLACE FUNCTION fn_seguimiento_diario_produccion(

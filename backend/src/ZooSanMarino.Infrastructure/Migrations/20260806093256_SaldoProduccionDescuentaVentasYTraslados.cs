...
{
    /// <summary>
    /// El saldo de aves de PRODUCCIÓN descuenta ventas, retiros, traslados y la selección de machos.
    ///
    /// <para>
    /// <c>fn_indicadores_produccion_postura</c> hacía
    /// <c>v_aves_h_act := GREATEST(0, v_aves_h_act - r_mort_h - r_sel_h)</c> y, para machos, solo
    /// restaba la mortalidad. Faltaban tres cosas:
    /// </para>
    /// <list type="number">
    ///   <item>Las <b>ventas</b>: una venta de producción no deja columna numérica en la fila diaria
    ///     —solo una nota en observaciones— así que quedaba fuera del saldo. Medido en el lote
    ///     S-369: <b>+114 hembras</b> en un sublote y <b>+224</b> en el otro.</item>
    ///   <item>Los <b>traslados</b> de aves (salidas e ingresos).</item>
    ///   <item>La <b>selección de machos</b>, que la fn no leía en ningún lado: ni para el saldo ni
    ///     para la salida (<c>SeleccionMachos</c> del reporte estaba fijo en 0). Eran otras
    ///     <b>61</b> y <b>77</b> aves de más.</item>
    /// </list>
    ///
    /// <para>
    /// Los datos ya estaban: <c>fn_seguimiento_diario_produccion</c> expone <c>sel_m</c>,
    /// <c>mov_venta_*</c>, <c>mov_retiro_*</c> y <c>mov_traslado_in/out_*</c> (agrega
    /// <c>movimiento_aves</c> por día). Esta migración solo los sube al saldo, con la misma
    /// composición que <c>SaldoAvesLevanteCalculos.BajasNetas</c>, su especificación ejecutable.
    /// El % de retiro de machos pasa a incluir la selección, igual que el de hembras.
    /// </para>
    ///
    /// <para>
    /// La firma cambia (columna nueva <c>seleccion_machos</c>), así que va con DROP + CREATE en vez
    /// de CREATE OR REPLACE. Idempotente por el <c>DROP FUNCTION IF EXISTS</c>; <c>Down()</c>
    /// restituye la versión previa, firma incluida.
    /// </para>
    /// </summary>
    public partial class SaldoProduccionDescuentaVentasYTraslados : Migration
    {
        private const string DropFn =
            "DROP FUNCTION IF EXISTS public.fn_indicadores_produccion_postura(integer,integer,integer,integer,integer,date,date);";

        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(DropFn);
            migrationBuilder.Sql(FnConSaldoCorregido);
        }
        ...
        {
            migrationBuilder.Sql(DropFn);
            migrationBuilder.Sql(FnPrevia);
        }

        private const string FnConSaldoCorregido = """
...
DECLARE
    ...
    -- solo restaba mortalidad y seleccion, asi que una venta de produccion —que no deja
    -- columna numerica en la fila diaria, solo nota— quedaba fuera y el saldo del
    -- reporte terminaba por encima del real en exactamente el total vendido.
    r_sel_m          integer;   -- la fn nunca llevo la seleccion de machos: ni al saldo ni a la salida
    r_venta_h        integer;
    r_venta_m        integer;
    r_retiro_h       integer;
    r_retiro_m       integer;
...
BEGIN
    ...
    FOR s IN 25..v_max_sem LOOP  -- REQ-012b: incluir semana 25 (antes 26)
        ...
        -- Decremento de aves. Ademas de mortalidad y seleccion descuenta VENTAS, retiros
        -- y salidas por traslado, y suma los ingresos: son aves que dejan (o entran a)
        -- el lote igual que las bajas. Misma composicion que SaldoAvesLevanteCalculos.

...
{
    /// <summary>
    /// `fn_indicadores_produccion_postura` emite ahora `porcentaje_seleccion_machos`.
    ///
    /// <para>
    /// La selección de machos ya se contaba (`seleccion_machos`, agregada por
    /// <c>20260806093256_SaldoProduccionDescuentaVentasYTraslados</c>), ya se descontaba del saldo y
    /// ya entraba al %retiro de machos — pero su <b>porcentaje</b> nunca existió: la fn solo calculaba
    /// `porcentaje_seleccion_hembras`, así que la tabla de indicadores mostraba «%Sel H» sin su par.
    /// </para>
    ///
    /// <para>
    /// Cambio <b>aditivo</b>: una columna nueva al final del bloque de selección y una variable que
    /// se calcula con la <b>misma fórmula y el mismo denominador</b> que la de hembras (el saldo del
    /// sexo antes de descontar la semana, `v_aves_m_act`). Ninguna otra columna se toca — el resto
    /// del cuerpo es byte a byte el de <c>20260807140000_UniformidadGuiaProduccionNull</c>.
    /// </para>
    ///
    /// <para>
    /// La firma cambia ⇒ va con <c>DROP + CREATE</c> en vez de <c>CREATE OR REPLACE</c> (Postgres no
    /// permite cambiar el tipo de retorno). Idempotente por el <c>DROP FUNCTION IF EXISTS</c>;
    /// <c>Down()</c> restituye la versión previa completa.
    /// </para>
    /// </summary>
    public partial class PorcentajeSeleccionMachosProduccion : Migration
    {
        /// <summary>Firma vieja y nueva coinciden en los parámetros, así que el DROP sirve para ambas.</summary>
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(FnConPorcentajeSeleccionMachos);
        }
        ...
        private const string FnConPorcentajeSeleccionMachos = """
CREATE FUNCTION fn_indicadores_produccion_postura(
...
RETURNS TABLE(
    ...
    -- El conteo viajaba desde 20260806093256 pero su porcentaje no existía: la tabla mostraba
    --   %Sel solo para hembras. Mismo denominador que el de hembras (saldo del sexo pre-decremento).
    porcentaje_seleccion_machos         double precision,

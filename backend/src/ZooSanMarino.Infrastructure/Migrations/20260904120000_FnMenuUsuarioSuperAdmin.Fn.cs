// Partial de la migracion FnMenuUsuarioSuperAdmin: las constantes SQL, para que el archivo
// principal se pueda leer.
//
//   FnMenuUsuarioSuperAdminSql -> backend/sql/fn_menu_usuario.sql TAL CUAL (espejo, con D5).
//   FnMenuUsuarioSinBypassSql  -> la version anterior (20260826120000_FnMenuUsuario), solo para Down().

#nullable disable

namespace ZooSanMarino.Infrastructure.Migrations
{
    public partial class FnMenuUsuarioSuperAdmin
    {
        /// <summary>
        /// El menu efectivo del usuario en su empresa, con el bypass del gate de empresa para el
        /// super admin (D5). Espejo exacto de backend/sql/fn_menu_usuario.sql.
        /// </summary>
        private const string FnMenuUsuarioSuperAdminSql = """
-- ============================================================================
...
--
--   D5 — El gate de empresa NO se le aplica al super admin (users.is_super_admin).
--        (4-sep-2026, plan: fase_de_desarrollo/soporte_sanmarino_y_admin_global_plan.md)
--        Es el único que puede pararse en cualquier empresa, y su menú se armaba con el
--        company_menus de la empresa activa como el de cualquiera. Los ítems que administran
--        el sistema entero —Empresas y db_studio— viven habilitados en UNA SOLA empresa, así
--        que quitárselos a esa empresa lo dejaba sin el módulo Empresas en toda la app y sin
--        ruta de vuelta por la UI: para rehabilitarlo hay que entrar a Configuración →
--        Empresas → Menús, que es justo el menú que desapareció. D2 no cubre el caso: su
--        fail-open aplica a la empresa SIN ninguna fila, y las cinco tienen.
--
--        Recorta el gate de EMPRESA y nada más: role_menus, menus.is_active y menu_permissions
--        le siguen aplicando igual que a cualquiera. Para todo usuario sin la marca, el
--        resultado es idéntico al previo.
--
...
BEGIN
    ...
    -- D5: y nunca al super admin (users.is_super_admin) — ver el encabezado.
    empresa_filtra AS (
        SELECT p_company_id IS NOT NULL
           AND EXISTS (SELECT 1 FROM company_menus cm WHERE cm.company_id = p_company_id)
           AND NOT COALESCE(
                   (SELECT u.is_super_admin FROM users u WHERE u.id = p_user_id),
                   false)                                    -- usuario inexistente ⇒ NO es super admin
               AS si
    ),
...
COMMENT ON FUNCTION fn_menu_usuario(uuid, integer) IS
    'Menu efectivo de un usuario en una empresa, ya armado como arbol jsonb. Interseca role_menus, menu_permissions y company_menus (este ultimo solo si la empresa tiene configuracion Y el usuario NO es super admin: regla D5). Espejo: backend/sql/fn_menu_usuario.sql';
""";

        /// <summary>
        /// La version SIN el bypass, tal como la dejo 20260826120000_FnMenuUsuario. Solo la usa
        /// Down(): al super admin vuelve a aplicarsele el gate de company_menus.
        /// </summary>
        private const string FnMenuUsuarioSinBypassSql = """
-- ============================================================================
-- fn_menu_usuario — el menú EFECTIVO de un usuario dentro de una empresa,
--                   ya armado como árbol jsonb.
-- ----------------------------------------------------------------------------

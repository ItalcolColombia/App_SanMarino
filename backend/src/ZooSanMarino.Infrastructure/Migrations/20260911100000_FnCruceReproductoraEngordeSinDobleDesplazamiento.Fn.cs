...
--   * HORA DE LLEGADA (28-ago-2026): si el lote pollo engorde tiene
--     hora_encasetamiento >= 13:00 las aves no alcanzan a consumir el dia del
--     encaset; el primer dia con registro es el siguiente y TODA la serie se corre
--     un dia (fecha destino = encaset + 1 + d). Espejo de
--     EncasetamientoCalculos.DiasDesplazamiento (C#). Sin hora informada el
--     desplazamiento es 0 y el SQL queda identico al previo.
--   * EL DESPLAZAMIENTO NO SE APLICA DOS VECES (11-sep-2026): la regla de arriba
--     vale cuando la reproductora SI capturo el dia del encaset (edad 0). Si la
--     reproductora ya arranco en la edad 1 —por la MISMA llegada tardia, que es lo
--     correcto— sumarle otro dia manda el registro del dia siguiente al encaset al
--     subsiguiente. El desplazamiento efectivo es entonces
--         GREATEST(0, v_desp - primera_edad_generada)
--     y el GREATEST es la regla, no una defensa: sin hora (desp 0) una primera edad
--     >= 1 NO puede correr la serie HACIA ATRAS sobre el dia del encaset.
--     Espejo de EncasetamientoCalculos.DesplazamientoCruce (C#).
...
DECLARE
    ...
    v_fecha_dest date;
BEGIN
    -- Fecha y HORA de encasetamiento del lote pollo engorde (padre), para fechar el destino.
    SELECT lae.fecha_encaset::date, lae.hora_encasetamiento
      INTO v_fecha_enc, v_hora_enc
      FROM lote_ave_engorde lae
     WHERE lae.lote_ave_engorde_id = p_lote_ave_engorde_id;

    -- Desde las 13:00 INCLUSIVE las aves llegaron tarde: no consumen el dia del encaset. Se corre
    -- la serie ENTERA en vez de descartar el dia 0, porque ese consumo es real y tiene que llegar
    -- al lote engorde. NULL >= time no es true, asi que un lote sin hora informada cae en el ELSE
    -- y da 0 => misma fecha destino que antes de esta version.
    v_desp := CASE WHEN v_hora_enc >= time '13:00' THEN 1 ELSE 0 END;

    -- Desplazamiento EFECTIVO: se resuelve en la primera edad que el cruce realmente genera (ver la
    -- regla en el encabezado). NULL = todavia no se genero ninguna.
    v_desp_efec := NULL;
    ...
    -- Se borra el cruce ENTERO antes del loop, no edad por edad dentro de el. Con la serie corrida
    -- por la hora de llegada, la fila nueva de la edad d cae sobre la fila VIEJA de la edad d+1, que
    -- todavia no se habia borrado: el indice unico por dia la rechazaba y no entraba ninguna. Es la
    -- misma trampa de "correr fechas +1 choca con el indice unico" que ya conociamos.
    DELETE FROM seguimiento_diario_aves_engorde
     WHERE lote_ave_engorde_id = p_lote_ave_engorde_id
       AND origen_cruce;
    ...
    FOR d IN 0..c_max_dias LOOP
        ...
        -- Edad 0 -> dia del encaset, salvo llegada tardia (v_desp = 1), donde va al siguiente.
        IF r.n_con = v_n_lotes AND v_n_lotes > 0 THEN
            -- Primera edad que se genera: fija el desplazamiento efectivo para TODA la serie. El
            -- loop va de 0 a 7 en orden ascendente, asi que esta es la menor edad con registro
            -- confirmado en todos los lotes reproductora.
            IF v_desp_efec IS NULL THEN
                v_desp_efec := GREATEST(0, v_desp - d);
            END IF;

            v_fecha_dest := COALESCE(v_fecha_enc + v_desp_efec + d, r.fecha_reg);
            ...
            INSERT INTO seguimiento_diario_aves_engorde (
                ...
                jsonb_build_object(
                    'origenCruce', true,
                    'edad', d,
                    'desplazamientoHora', v_desp_efec,
                    'lotesReproductora', r.lotes_json
                ),
                true, 'SYSTEM_CRUCE', now(),
                true, now(), 'SYSTEM_CRUCE'
            )
            -- El dia destino puede estar ocupado por un registro MANUAL. Pasa en los lotes creados
            -- antes de que el cruce mirara la hora: su serie quedaba un dia corrida y el operario
            -- capturo a mano el dia que ahora reclama el cruce. Sin esto, el INSERT viola el indice
            -- unico por dia UTC y la CONFIRMACION DE REPRODUCTORA falla entera -- el operario no
            -- puede confirmar y no tiene forma de saber por que. Se salta el dia (el manual ya lo
            -- cubre) y queda la traza en el log. En un lote nuevo esto no ocurre: los guardas C#
            -- impiden capturar a mano antes del primer dia con registro.
            ON CONFLICT DO NOTHING;

            GET DIAGNOSTICS v_insertadas = ROW_COUNT;
            IF v_insertadas = 0 THEN
                RAISE WARNING 'fn_cruce_reproductora_a_engorde: lote %, edad %: el dia % ya estaba ocupado por otro registro; el cruce de esa edad se omitio.',
                    p_lote_ave_engorde_id, d, v_fecha_dest;
            END IF;
        END IF;
    END LOOP;
END;

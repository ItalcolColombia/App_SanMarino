...
{
    /// <summary>
    /// Recreacion de <c>fn_reporte_diario_costos_postura</c> para que la granja vigente de cada dia
    /// salga de <c>fecha_traslado</c> y no del instante de registro. Documentacion en
    /// <c>20260831170000_FechaTrasladoLote.cs</c>.
    /// </summary>
    public partial class FechaTrasladoLote
    {
        private const string FN_COSTOS_CON_FECHA_TRASLADO = @"CREATE OR REPLACE FUNCTION fn_reporte_diario_costos_postura(
    p_company_id           INT,
...
IS 'v2. Reporte diario del área de costos para postura (levante + producción) por lote:galpón. '
   ...
   'CorteEtapaPosturaCalculos).';";
        ...
        private const string FN_COSTOS_PREVIA = @"CREATE OR REPLACE FUNCTION fn_reporte_diario_costos_postura(
    p_company_id           INT,
    p_granja_ids           INT[] DEFAULT NULL,
    p_regional             TEXT  DEFAULT NULL,
    p_lote_postura_base_id INT   DEFAULT NULL,

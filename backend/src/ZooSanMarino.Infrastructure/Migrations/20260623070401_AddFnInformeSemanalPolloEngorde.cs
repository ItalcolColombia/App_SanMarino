...
{
    /// <summary>
    /// Crea fn_informe_semanal_pollo_engorde: Informe Semanal Pollo de Engorde (Panamá).
    /// 1 fila por (lote, semana de vida). Reales desde seguimiento_diario_aves_engorde +
    /// movimiento_pollo_engorde (reutiliza fn_seguimiento_diario_engorde vía CROSS JOIN LATERAL).
    /// Columnas "Tabla" (guía genética) = NULL placeholder (no conectado todavía).
    /// Fuente canónica: backend/sql/fn_informe_semanal_pollo_engorde.sql
    /// </summary>
    public partial class AddFnInformeSemanalPolloEngorde : Migration
    {
        /// <inheritdoc />

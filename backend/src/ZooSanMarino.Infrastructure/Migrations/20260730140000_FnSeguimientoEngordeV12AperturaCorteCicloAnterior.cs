...
{
    /// <summary>
    /// <c>fn_seguimiento_diario_engorde</c> v12: la ventana de apertura tampoco puede retroceder mas
    /// alla del FIN del ciclo anterior del galpon.
    /// <para>
    /// La v11 excluye los movimientos atribuidos a un lote AJENO, pero eso tapa solo la mitad del
    /// agujero. <c>lote_ave_engorde_id</c> lo pone el trigger con
    /// <c>fn_lote_ave_engorde_id_desde_ubicacion</c>, que devuelve el lote de id MAS ALTO del galpon
    /// al momento de INSERTAR, asi que la atribucion falla en los dos sentidos y hacen falta los dos
    /// criterios:
    /// </para>
    /// <list type="bullet">
    /// <item>limpieza etiquetada con el lote VIEJO (se registro antes de crear el nuevo) la caza
    /// <c>lotes_ajenos</c>, v11 — caso Kilometro 22 / G0036;</item>
    /// <item>limpieza etiquetada con el lote NUEVO (se registro despues) la caza este corte — caso
    /// SAN GUILLERMO / G0033: dos <c>INV_TRASLADO_SALIDA</c> del 13/03 por 960 + 4.200 = 5.160 kg que
    /// son el vaciado del ciclo 2601, cuyo ultimo seguimiento fue ESE MISMO 13/03, pero quedaron con
    /// el id del lote 2602. Para la v11 son «propios» y no los ve.</item>
    /// </list>
    /// <para>
    /// Nuevo CTE <c>corte_apertura</c>: la ventana arranca en
    /// <c>GREATEST(fecha_encaset − dias_alimento_previo_encaset, fin_del_ciclo_anterior + 1)</c>.
    /// </para>
    /// <para>
    /// <b>Verificado fila a fila sobre el dump de produccion:</b> ItalcolPanama 0 diferencias en saldo,
    /// aves, ingreso y documento (NO-OP exacto); ItalcolEcuador 330 filas de saldo corregidas, ninguna
    /// fila perdida (5.495 antes y despues) y las filas con saldo NEGATIVO bajan de 99 en 10 lotes
    /// (−315.441 kg) a 25 en 5 lotes (−146.991 kg). No toca el caso legitimo de v9 —el preiniciador que
    /// llega antes del encaset— porque ese llega mucho despues de que cerro el ciclo previo.
    /// </para>
    /// Idempotente (CREATE OR REPLACE). <c>Down</c> restaura la v11 completa.
    ...
    /// </summary>
    public partial class FnSeguimientoEngordeV12AperturaCorteCicloAnterior : Migration
    {
        /// <inheritdoc />

...
{
    /// <summary>
    /// Despliega fn_reporte_semanal_levante_extras(p_lote_id): complemento por
    /// SEXO (conteos/kg/pesos/uniformidad/nutricion) del Reporte Tecnico Semanal
    /// de LEVANTE (Sanmarino postura). Hermana de fn_indicadores_levante_postura
    /// (misma semana/guards/saldo). Espejo .sql: backend/sql/fn_reporte_semanal_levante_extras.sql.
    /// Data-only (Designer clonado, ModelSnapshot intacto). Idempotente
    /// (DROP FUNCTION IF EXISTS + CREATE OR REPLACE).
    /// </summary>
    public partial class AddFnReporteSemanalLevanteExtras : Migration
    {
        /// <inheritdoc />

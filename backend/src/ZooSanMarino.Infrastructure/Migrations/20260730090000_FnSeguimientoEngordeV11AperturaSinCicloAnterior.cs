...
{
    /// <summary>
    /// <c>fn_seguimiento_diario_engorde</c> v11: la apertura de alimento deja de heredar el CICLO
    /// ANTERIOR del galpon.
    /// <para>
    /// La ventana de alimento previo al encaset (v9) retrocede <c>dias_alimento_previo_encaset</c>
    /// dias. En Ecuador cada galpon encadena 3-4 ciclos sucesivos, asi que esa ventana cae DENTRO
    /// del ciclo anterior, justo cuando se vacia su bodega. Y como el filtro de devoluciones
    /// descarta las entradas (<c>devolucion por eliminacion</c>) pero conserva los traslados de
    /// salida, la apertura quedaba NEGATIVA.
    /// </para>
    /// <para>
    /// Caso testigo Kilometro 22 / G0036 / lote 98 «2603»: apertura −7.960 kg (+160 −7.520 −440
    /// −160, los cuatro del lote 65 «2602», que cerro el 01/06 y se vacio entre el 05 y el 07/06).
    /// La grilla mostraba 3.560 kg el primer dia contra 11.520 esperados, y 3.420 al cierre contra
    /// 11.380 kg de stock real: el MISMO desvio en las 43 filas, porque es un corrimiento de la
    /// apertura y no un error que se acumule.
    /// </para>
    /// <para>
    /// Es estructural y recurrente: la ventana solo alcanza la limpieza del ciclo anterior si ese
    /// ciclo existe, asi que aparece desde el TERCER ciclo de cada galpon. Ecuador va por la
    /// corrida 4 y tiene 26 lotes con apertura negativa (−98.692 kg); Panama hoy tiene 0 casos pero
    /// le pasaria lo mismo en cuanto encadene un tercer ciclo por galpon.
    /// </para>
    /// <para>
    /// El CTE nuevo <c>lotes_ajenos</c> es el COMPLEMENTO exacto del predicado que la v10 ya usa
    /// para repartir el consumo: un lote del mismo galpon cuyo rango de seguimiento no se solapa
    /// con el mio no comparte bodega conmigo. Los movimientos SIN atribucion
    /// (<c>lote_ave_engorde_id IS NULL</c>) se conservan: no se pierde alimento real.
    /// </para>
    /// <para>
    /// <b>El filtro va SOLO en <c>apert_mov</c></b>, y es a proposito. <c>lote_ave_engorde_id</c> no
    /// distingue ciclos: el sistema etiqueta el movimiento con el lote VIGENTE del galpon, asi que
    /// el preiniciador del ciclo nuevo queda con el id del viejo mientras el nuevo no tenga
    /// seguimiento cargado (CAROLINA G0059: 2.800 kg del 16/06 atribuidos al lote 63 siendo del 96).
    /// Por eso la propiedad solo puede usarse ANTES del primer seguimiento; dentro del rango propio
    /// del lote todo lo que entra al galpon es suyo. Filtrar tambien <c>hist_alimento</c> rompia
    /// esos 4 galpones de CAROLINA.
    /// </para>
    /// <para>
    /// <b>Verificado fila a fila sobre el dump de produccion</b> (5.619 filas, 134 lotes):
    /// ItalcolPanama 0 diferencias en saldo, aves, ingreso y documento (NO-OP exacto);
    /// ItalcolEcuador 1.425 filas de saldo corregidas y ninguna fila de seguimiento perdida
    /// (5.495 antes y despues). Las 24 filas que desaparecen son filas de movimiento suelto de otro
    /// ciclo, sin seguimiento asociado.
    /// </para>
    /// Idempotente (CREATE OR REPLACE). <c>Down</c> restaura la v10 completa.
    ...
    /// </summary>
    public partial class FnSeguimientoEngordeV11AperturaSinCicloAnterior : Migration
    {
        /// <inheritdoc />

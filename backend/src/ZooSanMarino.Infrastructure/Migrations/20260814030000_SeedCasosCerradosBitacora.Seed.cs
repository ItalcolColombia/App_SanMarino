...
";
        ...
        private const string DOWN_SQL = @"-- Revierte SOLO los casos de esta migración, identificados por la marca de la descripción.
...
BEGIN
    UPDATE public.ticket_tareas SET ticket_id = NULL
     WHERE ticket_id IN (SELECT id FROM public.tickets WHERE descripcion LIKE '[Bitácora jul-ago 2026]%');

    DELETE FROM public.tickets WHERE descripcion LIKE '[Bitácora jul-ago 2026]%';
END $$;
";
    }
}

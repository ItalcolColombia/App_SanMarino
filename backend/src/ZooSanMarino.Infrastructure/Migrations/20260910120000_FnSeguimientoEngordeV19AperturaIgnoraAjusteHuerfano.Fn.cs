// Partial de la migracion FnSeguimientoEngordeV19AperturaIgnoraAjusteHuerfano: SOLO las constantes
// SQL, para que el archivo principal se pueda leer. `...V19` es backend/sql/fn_seguimiento_diario_engorde.sql
// TAL CUAL (espejo); `...V18` es la version que habia antes, verbatim, para el Down.

#nullable disable

namespace ZooSanMarino.Infrastructure.Migrations
{
    public partial class FnSeguimientoEngordeV19AperturaIgnoraAjusteHuerfano
    {
        /// <summary>
        /// v19 - las 5 CTE que leen `INV_AJUSTE_CUADRE_ENTRADA`/`_SALIDA` descartan los que vienen
        /// sin lote: un ajuste de cuadre huerfano es la limpieza de un ciclo que ya no existe y no
        /// puede caer en la apertura del ciclo siguiente.
        /// Espejo exacto de backend/sql/fn_seguimiento_diario_engorde.sql.
        /// </summary>
        private const string FnSeguimientoDiarioEngordeV19 = """
-- =============================================================================
...
--
-- v19 (2026-09-10) — Un ajuste de cuadre SIN lote no es de ningun ciclo: nadie lo lee.
--   Ticket de operacion Panama: DOÑA MARIA / nucleo C / galpon 2 (`G0490`), lote 257 «61 - 1».
--   Stock 11.715,000 kg contra saldo de tabla 7.718,44 ⇒ 3.996,56 kg exactos de diferencia.
--   Plan: fase_de_desarrollo/apertura_engorde_ajuste_cuadre_huerfano_plan.md
--
--   QUE PASABA. El 05-sep 09:56 borraron (soft delete) los lotes 168 y 169, los unicos vivos de
--   `G0490`. A las 09:58 borraron el stock que sobraba del ciclo — 3.996,560 kg de AV. SUPER POLLO
--   ENGORDE. `EliminarStockAsync` escribe entonces un `AjusteCuadreTablaSalida` para que la baja
--   llegue tambien a la tabla diaria (el fix del 1-sep), y el trigger le pone el lote via
--   `fn_lote_ave_engorde_id_desde_ubicacion`, que filtra `deleted_at IS NULL` ⇒ devuelve NULL,
--   porque los candidatos se habian borrado 55 segundos antes. El 06-sep encasetaron el lote 257 y
--   la ventana de alimento previo al encaset (v9, 10 dias) recogio ese -3.996,56 como apertura.
--
--   Las dos guardas que existian no alcanzan: `lotes_ajenos` (v11) filtra por lote y su condicion
--   es `h.lote_ave_engorde_id IS NULL OR NOT EXISTS (…)` ⇒ con lote NULL es TAUTOLOGICA; y
--   `corte_apertura` (v12) busca el ciclo anterior con `l2.deleted_at IS NULL` ⇒ los lotes recien
--   borrados son invisibles y el piso cae de vuelta en `fecha_encaset − 10` = 27-ago.
--
--   LA REGLA. El fallback documentado en `fn_lote_ave_engorde_id_desde_ubicacion` («si no queda
--   lote vivo devuelve NULL y la apertura del ciclo siguiente las recoge») es CORRECTO para un
--   INGRESO —alimento que llego antes que los pollitos es del ciclo que entra— y EQUIVOCADO para
--   una SALIDA de cuadre: es la limpieza contable de un ciclo que ya no existe, no es de nadie, y
--   cobrarsela al siguiente le descuenta alimento que nunca recibio (encima de otro tipo:
--   SUPER POLLO ENGORDE contra el PREINICIADOR del ciclo nuevo).
--
--   EL ARREGLO. Las 5 CTE que leen `INV_AJUSTE_CUADRE_ENTRADA`/`_SALIDA` (`apert_mov`, `hist_full`,
--   `hist_alimento`, `docs_por_fecha`, `fechas_universo`) descartan los que vienen con
--   `lote_ave_engorde_id IS NULL`. Un ajuste CON lote se comporta exactamente igual que hoy.
--
--   POR QUE ESA CONDICION Y NO OTRA (las tres alternativas descartadas):
--     1. Excluir TODOS los `INV_AJUSTE_CUADRE_*` de la apertura ⇒ rompe el ajuste legitimo cargado
--        sobre un lote encasetado que todavia no cargo su dia 1.
--     2. Aceptar solo los del propio lote (`= p_lote_id`) ⇒ ROMPE UN GALPON QUE HOY CUADRA: en
--        `GALPON` (Doña Maria C-1) el ajuste del lote 254 esta en la ventana de apertura del 256 y
--        es correcto que entre (comparten bodega, v10). Excluirlo subia la apertura del 256 de
--        9.847 a 11.993,35 y su saldo final a 10.496,35 contra un stock de 8.350.
--     3. Cambiar el trato del `EliminacionStock` (`INV_OTRO`) ⇒ es el naufragio de v15/v16, que el
--        gate multipais revirtio DOS veces: mueve filas que ya existen.
--
--   RADIO. Las filas de ajuste de cuadre existen SOLO en Panama y SOLO desde el 05-sep-2026: 11
--   filas, 45.183,08 kg, 10 de ellas sin lote. El resto de las empresas da 0 por construccion.
--
...
apert_mov AS (
    ...
    WHERE NOT h.anulado
      ...
      -- ⭐ v19: un ajuste de cuadre SIN lote no es de NINGUN ciclo. Ver cabecera v19.
      AND NOT (h.tipo_evento IN ('INV_AJUSTE_CUADRE_ENTRADA', 'INV_AJUSTE_CUADRE_SALIDA')
...
""";
        ...
        /// <summary>
        /// v18 verbatim - la version que habia antes de v19, para el Down.
        /// </summary>
        private const string FnSeguimientoDiarioEngordeV18 = """
-- =============================================================================
-- fn_seguimiento_diario_engorde(p_lote_id INT)
-- Devuelve la tabla diaria de seguimiento de un lote de pollo engorde.
-- Tabla fuente: seguimiento_diario_aves_engorde

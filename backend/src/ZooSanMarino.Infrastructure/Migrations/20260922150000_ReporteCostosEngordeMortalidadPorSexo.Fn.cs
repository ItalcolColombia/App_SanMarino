// Partial de 20260922150000_ReporteCostosEngordeMortalidadPorSexo: SQL verbatim.
// V4 = espejo backend/sql/fn_reporte_diario_costos_engorde.sql (este commit); V3 = el espejo previo
// (= prosrc desplegado por 20260729224401_ReporteCostosAlimentoDesdeFuentesReales) para el Down.

namespace ZooSanMarino.Infrastructure.Migrations
{
    public partial class ReporteCostosEngordeMortalidadPorSexo
    {
        private const string FnReporteCostosEngordeV4 = """
-- =============================================================================
...
--
-- v4 (2026-09-22) — Mortalidad y selección POR SEXO en cada galpón (pedido de Ecuador).
--   * El JSON `galpones` suma 6 claves: mortalidad_hembras, mortalidad_machos, seleccion_hembras,
--     seleccion_machos, mort_sel_hembras, mort_sel_machos. Salen de las mismas columnas de
--     fn_seguimiento_diario_engorde que ya se sumaban (mortalidad_hembras/machos, sel_h/sel_m), así
--     que H + M == el valor combinado de siempre, galpón por galpón y día por día.
--   * NADA existente cambia: RETURNS TABLE, las claves previas del JSON, alimentos y totales quedan
--     byte a byte iguales (CREATE OR REPLACE alcanza, sin DROP).
--   * Quién ve el desglose NO lo decide la fn: el service lo gatea con
--     companies.seguimiento_engorde_mixto (Panamá guarda la mortalidad mixta en la columna H, así
--     que partirla mostraría un "macho 0" falso).
--
...
RETURNS TABLE (
    ...
    galpones         TEXT    -- [{galpon_id, galpon_nombre, mortalidad, seleccion, err_sexaje, mort_sel, consumo_kg, aves_vivas,
                             --   mortalidad_hembras, mortalidad_machos, seleccion_hembras, seleccion_machos,
                             --   mort_sel_hembras, mort_sel_machos}]
...
diario_full AS MATERIALIZED (
    SELECT
        ...
        COALESCE(f.mortalidad_hembras, 0)                                            AS mortalidad_h,
        COALESCE(f.mortalidad_machos, 0)                                             AS mortalidad_m,
        COALESCE(f.sel_h, 0)                                                         AS seleccion_h,
        COALESCE(f.sel_m, 0)                                                         AS seleccion_m,
...
galpon_fecha AS (
    SELECT
        ...
        SUM(d.mortalidad_h)       AS mortalidad_h,
        SUM(d.mortalidad_m)       AS mortalidad_m,
        SUM(d.seleccion_h)        AS seleccion_h,
        SUM(d.seleccion_m)        AS seleccion_m
    FROM diario d
...
galp_json AS (
    SELECT
        ...
        jsonb_agg(jsonb_build_object(
            ...
            -- v4: desglose por sexo (H + M == la clave combinada de arriba).
            'mortalidad_hembras', COALESCE(gf.mortalidad_h, 0),
            'mortalidad_machos',  COALESCE(gf.mortalidad_m, 0),
            'seleccion_hembras',  COALESCE(gf.seleccion_h, 0),
            'seleccion_machos',   COALESCE(gf.seleccion_m, 0),
            'mort_sel_hembras',   COALESCE(gf.mortalidad_h, 0) + COALESCE(gf.seleccion_h, 0),
            'mort_sel_machos',    COALESCE(gf.mortalidad_m, 0) + COALESCE(gf.seleccion_m, 0)
...
""";
        ...
        private const string FnReporteCostosEngordeV3 = """
-- =============================================================================
-- fn_reporte_diario_costos_engorde(p_company_id, p_granja_id, p_lote_base_id,
--                                  p_fecha_inicio, p_fecha_fin)
--

...
{
    /// <inheritdoc />
    public partial class AddFnReporteDiarioCostosPostura : Migration
    {
        // Reporte Diario Area de Costos de POSTURA (levante + produccion), por lote:galpon.
        // NO tiene relacion con fn_reporte_diario_costos_engorde: otras fuentes, otras reglas.
        //   - PRODUCCION: delega en fn_seguimiento_diario_produccion (fn diaria CANONICA) por LATERAL.
        //   - LEVANTE: no existe fn diaria canonica (fn_indicadores_levante_postura es SEMANAL), se
        //     lee seguimiento_diario_levante con el MISMO corte de dia America/Bogota.
        //   - Devuelve el huevo CRUDO: la agrupacion fertil/comercial/inservible vive en
        //     ReporteDiarioCostosPosturaCalculos (C# puro y testeado), no se duplica en SQL.
        //   - El nombre del alimento se resuelve contra los DOS catalogos vivos: el metadata del
        //     camino 1 solo trae catalogItemId (lotes viejos) y el del camino 2 trae nombre.
        // Idempotente: CREATE OR REPLACE, sin DDL de tablas ni cambios de modelo (ModelSnapshot intacto).
        // Fuente canonica: backend/sql/fn_reporte_diario_costos_postura.sql

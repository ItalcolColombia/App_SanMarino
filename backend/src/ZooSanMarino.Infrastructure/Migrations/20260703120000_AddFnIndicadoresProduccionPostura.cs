...
{
    /// <summary>
    /// Crea fn_indicadores_produccion_postura (+ helpers fn_dif_pct, fn_parse_edad_numerica):
    /// indicadores semanales de PRODUCCIÓN postura calculados en la BD, para que
    /// IndicadoresProduccionService delegue el cálculo (SqlQueryRaw) en vez de calcular en C#.
    /// Idempotente (CREATE OR REPLACE / DROP IF EXISTS). Migración hecha a mano (no altera el
    /// ModelSnapshot). Fuente/spec: backend/sql/fn_indicadores_produccion_postura.sql.
    /// </summary>
    public partial class AddFnIndicadoresProduccionPostura : Migration
    {
        /// <inheritdoc />
...
DROP FUNCTION IF EXISTS fn_parse_edad_numerica(text);
DROP FUNCTION IF EXISTS fn_dif_pct(double precision, double precision);
");
        }
    }
}

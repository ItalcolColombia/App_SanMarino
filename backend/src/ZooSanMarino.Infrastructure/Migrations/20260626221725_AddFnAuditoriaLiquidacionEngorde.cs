...
{
    /// <inheritdoc />
    public partial class AddFnAuditoriaLiquidacionEngorde : Migration
    {
        // Verificador de liquidación pollo engorde (Ecuador): reconciliación sistema vs Excel,
        // detectores de datos (despachos sin peso, anulados activos, multi-lote, ajuste alto, Excel
        // incompleto) y simulación de corrección. Toda la lógica en BD. Idempotente (CREATE OR REPLACE).
        /// <inheritdoc />
        ...
        {
            migrationBuilder.Sql("DROP FUNCTION IF EXISTS public.fn_auditoria_liquidacion_engorde(INT, INT, TEXT, TEXT, JSONB);", suppressTransaction: true);
        }

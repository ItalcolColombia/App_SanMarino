...
{
    /// <summary>
    /// <c>uniformidad_guia</c> de producción deja de coalescearse a 0: pasa a NULL cuando la guía
    /// no trae el dato.
    ///
    /// <para>
    /// La guía genética <b>no define uniformidad para las edades de PRODUCCIÓN</b> (solo la traen
    /// las filas de levante). El <c>COALESCE(g_unif, 0)</c> replicaba un <c>ParseDouble =&gt; 0</c>
    /// viejo del C#, así que la columna «Uniformidad Guía» mostraba <b>0 en todas las semanas</b>,
    /// que se lee como «la guía exige 0 %» en vez de «sin dato». Un 0 real tampoco existe como
    /// objetivo de uniformidad, así que NULL es la única lectura honesta.
    /// </para>
    ///
    /// <para>
    /// <c>diferencia_uniformidad</c> <b>no se mueve</b>: <c>fn_dif_pct</c> ya devolvía NULL cuando
    /// la guía era 0. Los demás campos de guía (<c>g_cons_*</c>, <c>g_mort_*</c>, <c>g_peso_*</c>,
    /// <c>g_retiro_ac_*</c>) conservan el 0 a propósito: la guía sí los trae en toda la curva y
    /// cambiarlos movería números sin necesidad.
    /// </para>
    ///
    /// <para>
    /// ⚠️ Esta migración además <b>reconcilia el espejo</b>
    /// <c>backend/sql/fn_indicadores_produccion_postura.sql</c>, que había quedado desincronizado:
    /// le faltaba todo lo que agregó <c>20260806093256_SaldoProduccionDescuentaVentasYTraslados</c>
    /// (columna <c>seleccion_machos</c>, ventas/retiros/traslados en el saldo). Correr ese archivo
    /// dejaba la fn en <b>68 columnas en vez de 69</b> y rompía
    /// <c>IndicadorProduccionSemanalBdRow.SeleccionMachos</c> en runtime. El cuerpo de esta
    /// migración sale del espejo YA reconciliado, así que a partir de acá archivo y BD coinciden.
    /// </para>
    ///
    /// <para>
    /// Gate de paridad (local, 5 empresas × 8 LPP + 6 lotes × ventana completa y semanas 30-40,
    /// 179 filas): el espejo reconciliado <b>con</b> el COALESCE restaurado da <c>EXCEPT</c> 0 en
    /// los dos sentidos y <b>0 diferencias en las 68 columnas</b> contra la fn desplegada ⇒ el port
    /// es fiel. Sin el COALESCE, la única columna que se mueve es <c>uniformidad_guia</c>, siempre
    /// <c>0 → NULL</c> (0 valores reales perdidos).
    /// </para>
    ///
    /// <para>
    /// La firma NO cambia (<c>seleccion_machos</c> ya está desplegada) ⇒ <c>CREATE OR REPLACE</c>
    /// sin DROP, idempotente. <c>Down()</c> restituye el comportamiento previo.
    /// </para>
    /// </summary>
    public partial class UniformidadGuiaProduccionNull : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder) =>
            migrationBuilder.Sql(FnUniformidadGuiaNull);

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder) =>
            migrationBuilder.Sql(FnPrevia);

        private const string FnUniformidadGuiaNull = """

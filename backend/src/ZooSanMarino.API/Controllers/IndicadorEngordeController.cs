// src/ZooSanMarino.API/Controllers/IndicadorEngordeController.cs
...
[ApiController]
// Ruta neutra: este módulo NO es de Ecuador — su propio front ya contiene los componentes
// `liquidacion-reporte-panama` y `corridas-panama`, y sus endpoints trabajan sobre lotes de engorde
// de cualquier empresa.
[Route("api/IndicadorEngorde")]
// Ruta histórica, conservada como ALIAS para el front ya desplegado.
[Route("api/IndicadorEcuador")]
[Authorize]
public class IndicadorEngordeController : ControllerBase
{
    private readonly IIndicadorEngordeService _indicadorService;
    private readonly ICurrentUser _current;
    private readonly ILogger<IndicadorEngordeController> _logger;

    public IndicadorEngordeController(
        IIndicadorEngordeService indicadorService,
        ICurrentUser current,
        ILogger<IndicadorEngordeController> logger)
    {
    ...
    /// </summary>
    [HttpPost("calcular")]
    public async Task<ActionResult<IEnumerable<IndicadorEngordeDto>>> CalcularIndicadores(
        [FromBody] IndicadorEngordeRequest request)
    {
        try
        {
    ...
    public async Task<ActionResult<IndicadorEngordeConsolidadoDto>> CalcularConsolidado(
        [FromBody] IndicadorEngordeRequest request)
    {
        try
        {
    ...
    public async Task<ActionResult<IEnumerable<IndicadorEngordeDto>>> ObtenerLotesCerrados(

...
[ApiController]
// Ruta neutra: esta guía NO es de Ecuador. La tabla tiene `pais_id` y guarda la guía de todas las
// empresas y países — la Ross 308 AP de Panamá vive acá. Es la guía de POLLO ENGORDE, distinta de
// `/api/guia-genetica` (levante), que es otro recurso y por eso el nombre lleva el módulo, no el país.
[Route("api/guia-genetica-engorde")]
// Ruta histórica, conservada como ALIAS para el front ya desplegado y para lo que la PWA tenga
// cacheado con esa clave.
[Route("api/guia-genetica-ecuador")]
[Produces("application/json")]
[Tags("GuiaGeneticaEngorde")]
public class GuiaGeneticaEngordeController : ControllerBase
{
    private readonly IGuiaGeneticaEngordeService _service;
    private readonly ILogger<GuiaGeneticaEngordeController> _logger;

    public GuiaGeneticaEngordeController(
        IGuiaGeneticaEngordeService service,
        ILogger<GuiaGeneticaEngordeController> logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    [HttpGet("filters")]
    [ProducesResponseType(typeof(GuiaGeneticaEngordeFiltersDto), StatusCodes.Status200OK)]
    public async Task<ActionResult<GuiaGeneticaEngordeFiltersDto>> GetFilters(CancellationToken ct)
    {
    ...
    [HttpGet("datos")]
    [ProducesResponseType(typeof(IEnumerable<GuiaGeneticaEngordeDetalleDto>), StatusCodes.Status200OK)]
    public async Task<ActionResult<IEnumerable<GuiaGeneticaEngordeDetalleDto>>> GetDatos(
        [FromQuery] string raza,
        [FromQuery] int anioGuia,
    ...
    [ProducesResponseType(typeof(GuiaGeneticaEngordeImportResultDto), StatusCodes.Status200OK)]
    public async Task<ActionResult<GuiaGeneticaEngordeImportResultDto>> Import(
        IFormFile file,
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new GuiaGeneticaEngordeImportResultDto(false, 0, 0, 1, new[] { ex.Message }));
        }
    }

    /// <summary>Alta/actualización manual de filas por sexo (reemplaza el detalle de ese sexo).</summary>
    [HttpPost("manual")]
    [ProducesResponseType(typeof(GuiaGeneticaEngordeHeaderDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<GuiaGeneticaEngordeHeaderDto>> Manual(
        [FromBody] GuiaGeneticaEngordeManualRequestDto body,
        CancellationToken ct)
    {
        try
        {

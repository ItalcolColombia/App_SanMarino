-- =============================================================================
-- fn_informe_semanal_pollo_engorde — Informe Semanal Pollo de Engorde (Panamá)
-- -----------------------------------------------------------------------------
-- Reporte semanal: una fila por (lote, semana de vida 1..N) con los indicadores
-- REALES tomados de seguimiento_diario_aves_engorde + movimiento_pollo_engorde.
-- Reutiliza fn_seguimiento_diario_engorde(lote) vía CROSS JOIN LATERAL: esa
...
--   p_granja_ids  : NULL o {} = todas las granjas; si no, sólo esas
--   p_nucleo_id   : NULL = todos
--   p_galpon_id   : NULL = todos
--   p_lote_id     : NULL = todos los que pasen el resto de filtros
--   p_fecha_desde : NULL = sin tope inferior (semana calendario; overlap)
...
RETURNS TABLE (
    ...
    -- % comparación real semanal vs tabla semanal (real/tabla * 100; NULL si no hay tabla)
    pct_consumo           NUMERIC,
    pct_peso              NUMERIC,
...
-- Diario del lote vía la función ya existente (hereda toda la lógica de saldos/ventas)
diario AS (
    SELECT
        lo.lote_id,
        d.semana,
        ...
        -- Peso del día (g): promedio de H/M no nulos; si sólo uno, ese
        CASE
...
-- Estándar semanal de la guía genética Ecuador (sexo='mixto', SUMA de los 7 días de la semana de vida).
-- Si el lote no tiene raza/ano_tabla_genetica configurado o no existe un header activo, la fila
-- simplemente no aparece aquí; el LEFT JOIN final devuelve NULL para esos lotes/semanas.
tabla_gen AS (
    SELECT
        lo.lote_id,
...
SELECT
    ...
    w.ventas_unid,
    -- % sobre saldo vivo al inicio de la semana
    ...
    w.peso_anterior_g,
    lo.peso_llegada_g,
    -- Ganancia: peso(sem) - peso(sem-1); en la 1ª usa peso de llegada
    CASE
    ...
    -- Conversión real = consumo acumulado por ave (g) / peso (g)
    ...
    -- Tabla genética: valores del estándar para la semana (SUMA de los 7 días, sexo='mixto')
    tg.consumo_tabla_g,
    tg.peso_tabla_g,
    tg.ganancia_tabla_g,
    tg.conversion_tabla,
    tg.mortalidad_tabla_pct,
    -- % real semanal vs tabla semanal (real/tabla * 100)

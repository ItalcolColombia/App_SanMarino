-- =============================================================================
-- Vista Power BI: vw_seguimiento_pollo_engorde — seguimiento diario pollo de engorde
-- =============================================================================
-- VOLCADO FIEL DE LA VISTA DESPLEGADA (67 columnas), regenerado el 2026-08-11 desde la BD
-- con la migracion 20260812021716_AddConsumoMixtoVistaPowerbiEngorde ya aplicada.
--
-- Antes de esta fecha el archivo definia `vw_seguimiento_pollo_engorde_unificado` y le faltaban
-- 14 columnas que la vista viva SI tenia (tipo_fila, uniformidad_*, cv_*, consumo_agua_ph/orp/
-- temperatura, ciclo, historico_consumo_alimento, despacho_peso_*, created_by_user_id):
-- aplicarlo borraba esas columnas y rompia los tableros. Ya no: esto es lo que hay desplegado.
--
-- COMO REGENERARLO (unica forma correcta de actualizar este archivo):
--   psql ... -t -A -c "SELECT pg_get_viewdef('public.vw_seguimiento_pollo_engorde'::regclass, true);"
--
-- COMO CAMBIAR LA VISTA: NO editando este archivo, sino con una migracion EF que ENVUELVA la
-- definicion viva (patron de 20260812021716_...): `CREATE OR REPLACE VIEW %I AS SELECT v.*,
-- <nuevas> FROM (pg_get_viewdef(...)) v`. Asi cada ambiente conserva lo suyo y solo se agregan
-- columnas AL FINAL, que es lo unico que CREATE OR REPLACE VIEW permite. Despues se regenera
-- este archivo con el comando de arriba.
--
-- SEGURIDAD AL APLICARLO: es CREATE OR REPLACE (no DROP). Si el ambiente destino tuviera columnas
-- distintas, Postgres FALLA con error en vez de borrar nada. Falla segura, no destructiva.
--
-- ANIDACION (v_1_1 / v_1 / v): no es un error de formato. Cada capa la agrego una migracion que
-- envolvio la definicion anterior para sumar columnas al final sin tocar las existentes.
--
-- CONSUMO DE ALIMENTO — leer antes de graficar:
--   * consumo_real_dia_kg  -> el total del dia. La columna correcta para CUALQUIER suma.
--   * consumo_es_mixto = false -> dias 1-7 (cruce de lotes reproductora): el desglose
--                                 consumo_kg_hembras / consumo_kg_machos es real.
--   * consumo_es_mixto = true  -> del dia 8 en adelante el galpon come UNA SOLA racion; el total
--                                 esta en consumo_kg_mixto (consumo_kg_hembras lo repite solo por
--                                 como se persiste, y consumo_kg_machos vale 0: no los uses ahi).
--
-- Filtro obligatorio en Power BI: company_id (la vista devuelve todas las empresas).
-- =============================================================================

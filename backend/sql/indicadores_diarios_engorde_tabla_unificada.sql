-- =============================================================================
-- Indicadores diarios pollo engorde — alineados al tab "Indicadores" (vs guía Ecuador mixto)
-- =============================================================================
-- Parámetros (CTE `params`):
--   company_id     — obligatorio (ej. compañía espejo: 2)
--   lote_ave_engorde_id — opcional: NULL = todos los lotes de esa compañía; o un id concreto
--
-- Fuentes:
--   • public.lote_ave_engorde (+ companies, farms, nucleos, galpones)
--   • public.seguimiento_diario_aves_engorde (agregado por día de calendario)
--   • public.guia_genetica_ecuador_header + public.guia_genetica_ecuador_detalle (sexo = mixto)
--
-- Diferencias respecto al front (TypeScript):
--   • Consumo kg del día: SUM(consumo_kg_hembras + consumo_kg_machos). En la app, si hay
--     hembras + ítems "generales" en metadata, usa otra ruta; para DWH usar esta suma o
--     ampliar extrayendo jsonb en otra versión.
--   • Ganancia diaria: peso_mixto_día − peso_mixto del día anterior (aprox.); el front usa
--     último peso medido no nulo (misma idea en la práctica si hay un registro/día).
--
-- Uso:
--   Ajuste `params` y ejecute la consulta completa.
-- =============================================================================

WITH
params AS (
  SELECT
    2::int AS company_id,              -- <<<<<<<<<< compañía
    NULL::int AS lote_ave_engorde_id   -- <<<<<<<<<< NULL = todos; o p.ej. 12345
),
...
lote_filtrado AS (
  ...
  INNER JOIN params p ON p.company_id = l.company_id
    AND (p.lote_ave_engorde_id IS NULL OR l.lote_ave_engorde_id = p.lote_ave_engorde_id)
...
FROM final f
ORDER BY f.company_id, f.lote_ave_engorde_id, f.fecha_registro;

-- =============================================================================
-- MIGRACIÓN: Lote 13 - Producción (LPP-H, LPP-M) + produccion_diaria → seguimiento_diario
--
-- Precondición: Ejecutar antes la migración de levante para lote 13
--   (migracion_seguimiento_levante_to_diario_lote_13.sql) para tener LPL con
--   aves_h_actual y aves_m_actual.
--
-- Este script:
--   1. Crea LPP-H y LPP-M desde el LPL del lote 13 (aves actuales del levante).
--   2. Cierra el LPL (estado_cierre = 'Cerrado').
--   3. Copia registros de produccion_diaria (lote 13) a seguimiento_diario
...
DO $$
DECLARE
  p_lote_id INT := 13;  -- Cambiar si se migra otro lote
  r_lpl RECORD;
  aves_h INT;
  aves_m INT;
  base_nombre TEXT;
...
BEGIN
  ...
  IF r_lpl.lote_postura_levante_id IS NULL THEN
    RAISE EXCEPTION 'No existe LPL para lote_id %. Ejecuta antes migracion_seguimiento_levante_to_diario_lote_13.sql', p_lote_id;
  END IF;
  ...
  -- 5) Copiar produccion_diaria → seguimiento_diario (lote 13 → LPP-H)
  --    produccion_diaria.lote_id puede ser INT o VARCHAR según BD; usamos ::text = '13'
...
-- Resumen
SELECT 'Lote 13 producción: LPP creados' AS paso;
...
FROM public.lote_postura_produccion lpp
WHERE lpp.lote_postura_levante_id = (SELECT lote_postura_levante_id FROM public.lote_postura_levante WHERE lote_id = 13 AND deleted_at IS NULL LIMIT 1)
...
WHERE sd.tipo_seguimiento = 'produccion'
  AND sd.lote_postura_produccion_id IN (
    ...
    WHERE lote_postura_levante_id = (SELECT lote_postura_levante_id FROM public.lote_postura_levante WHERE lote_id = 13 AND deleted_at IS NULL LIMIT 1)

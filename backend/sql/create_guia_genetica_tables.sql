...
CREATE TABLE IF NOT EXISTS public.guia_genetica_header (
...
CREATE TABLE IF NOT EXISTS public.guia_genetica_detalle (
    id SERIAL PRIMARY KEY,
    guia_genetica_header_id INTEGER NOT NULL,
    ...
    updated_at TIMESTAMPTZ NULL,
    deleted_at TIMESTAMPTZ NULL,
    CONSTRAINT uq_gge_det_header_sexo_dia UNIQUE (guia_genetica_header_id, sexo, dia),
    CONSTRAINT fk_gge_det_header FOREIGN KEY (guia_genetica_header_id)
        REFERENCES public.guia_genetica_header(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS ix_gge_det_header_id ON public.guia_genetica_detalle (guia_genetica_header_id);
CREATE INDEX IF NOT EXISTS ix_gge_header_company_id ON public.guia_genetica_header (company_id);

COMMENT ON TABLE public.guia_genetica_header IS 'Encabezado guía genética Ecuador (company + raza + año).';
COMMENT ON TABLE public.guia_genetica_detalle IS 'Curva por día y sexo (mixto/hembra/macho).';

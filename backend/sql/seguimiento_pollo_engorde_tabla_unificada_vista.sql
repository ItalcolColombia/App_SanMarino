...
-- (SeguimientoAvesEngordeService.RecalcularSaldoAlimentoPorLoteAsync):
--   • No resta INV_CONSUMO del histórico (consumo va en seguimiento diario).
--   • INV_INGRESO, traslados, INV_OTRO (AjusteStock / EliminacionStock).
--   • Fecha efectiva en referencia (ymdHistoricoEfectivo).
--   • Filtro fecha encaset en tramo principal.
--   • Piso 0 tras cada movimiento; orden: apertura galpón → eventos principales.
--
-- saldo_alimento_kg_bd = columna persistida en seguimiento_diario_aves_engorde
--   (debe coincidir tras desplegar backend y recalcular).
-- saldo_alimento_kg_calculado = recalculado en SQL (consultas DWH sin API).
--
-- Columnas de contexto: company_id, empresa_nombre (companies.name), granja_id/nombre,
--   galpon_id/nombre, nucleo_id/nombre.
--
-- Consulta típica:
--   SELECT * FROM public.vw_seguimiento_pollo_engorde_unificado
--   WHERE company_id = 3
--   ORDER BY granja_id, lote_ave_engorde_id, fecha_registro;
-- =============================================================================

CREATE OR REPLACE VIEW public.vw_seguimiento_pollo_engorde_unificado AS
WITH RECURSIVE
hist_base AS (
  SELECT
    h.id AS hid,
    ...
    h.tipo_evento,
    h.created_at,
    trim(COALESCE(h.referencia, '') || ' ' || COALESCE(h.numero_documento, '')) AS ref_full,
    COALESCE(
      CASE
...
-- Apertura (antes del primer seguimiento): orden solo ymd + CreatedAt ms (ComputeSaldoAperturaGalponAntesPrimerSeguimiento).
hist_opening AS (
  SELECT
...
-- Tramo principal: ymd + ord (TryGetHistDeltaAndOrd) + tie ms; filtro fecha encaset como en C#.
hist_main AS (
  SELECT
    hb.lote_ave_engorde_id,
    1 AS phase,
...
-- TsSeguimiento (C#): mediodía UTC del día de Fecha; desempate entre filas del mismo día por id.
seg_events AS (
  SELECT
...
COMMENT ON VIEW public.vw_seguimiento_pollo_engorde_unificado IS
  'Seguimiento diario engorde + histórico por día. saldo_alimento_kg_calculado = misma lógica que la UI (sin INV_CONSUMO duplicado). saldo_alimento_kg_bd = valor persistido.';

...
-- Los ítems usan item_inventario (Configuración > Ítems inventario Ecuador).
-- Orden: 1) item_inventario (si no existe), 2) inventario_gestion_stock, 3) inventario_gestion_movimiento.
...
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS grupo VARCHAR(100) NULL;
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS tipo_inventario_codigo VARCHAR(50) NULL;
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS descripcion_tipo_inventario VARCHAR(200) NULL;
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS referencia VARCHAR(100) NULL;
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS descripcion_item VARCHAR(500) NULL;
ALTER TABLE public.item_inventario ADD COLUMN IF NOT EXISTS concepto VARCHAR(200) NULL;

-- 2) Stock: por granja o por granja+núcleo+galpón (alimento). Referencia a item_inventario.
CREATE TABLE IF NOT EXISTS public.inventario_gestion_stock (
    ...
    item_inventario_id INT NOT NULL,
    ...
    CONSTRAINT fk_igs_item_inventario FOREIGN KEY (item_inventario_id) REFERENCES public.item_inventario(id) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS ix_inventario_gestion_stock_farm_item_nucleo_galpon
    ON public.inventario_gestion_stock (farm_id, item_inventario_id, nucleo_id, galpon_id);
...
-- 3) Movimientos (ingresos y traslados). Referencia a item_inventario.
CREATE TABLE IF NOT EXISTS public.inventario_gestion_movimiento (
    ...
    CONSTRAINT fk_igm_item_inventario FOREIGN KEY (item_inventario_id) REFERENCES public.item_inventario(id) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS ix_igm_farm_item ON public.inventario_gestion_movimiento (farm_id, item_inventario_id);
...
COMMENT ON TABLE public.inventario_gestion_stock IS 'Stock del módulo Gestión de Inventario (Panama/Ecuador). Alimento: granja+núcleo+galpón; otros: solo granja. Ítems desde item_inventario.';
COMMENT ON TABLE public.inventario_gestion_movimiento IS 'Movimientos (ingresos/traslados/consumos) del módulo Gestión de Inventario. Ítems desde item_inventario.';
...
-- ALTER TABLE inventario_gestion_stock RENAME COLUMN catalog_item_id TO item_inventario_id;
-- ALTER TABLE inventario_gestion_stock DROP CONSTRAINT IF EXISTS fk_igs_catalog_item;
-- ALTER TABLE inventario_gestion_stock ADD CONSTRAINT fk_igs_item_inventario FOREIGN KEY (item_inventario_id) REFERENCES public.item_inventario(id) ON DELETE RESTRICT;
-- (y análogo para inventario_gestion_movimiento). Luego copiar datos de catalogo_items a item_inventario si aplica.

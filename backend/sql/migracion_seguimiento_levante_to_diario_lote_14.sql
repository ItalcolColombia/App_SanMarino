-- =============================================================================
-- MIGRACIÓN: Lote 14 - Levante a LPL + seguimiento_lote_levante → seguimiento_diario
-- Orden:
--   1. Crear lote_postura_levante para lote 14 si no existe.
--   2. Copiar registros de seguimiento_lote_levante (lote 14) a seguimiento_diario.
--   3. Actualizar LPL con aves_h_actual y aves_m_actual calculados desde seguimiento.
-- Nota: Ejecutar antes migracion_00_calculo_aves_lote_14.sql para validar los cálculos.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- PASO 1: Crear LPL para lote 14 si no existe (idempotente)
-- -----------------------------------------------------------------------------
...
WHERE l.lote_id = 14
  ...
  AND NOT EXISTS (
    ...
    WHERE lpl.lote_id = 14 AND lpl.deleted_at IS NULL
  );

-- -----------------------------------------------------------------------------
-- PASO 2: Copiar seguimiento_lote_levante (lote 14) → seguimiento_diario (idempotente)
...
FROM (
    ...
    WHERE lpl2.lote_id = 14 AND lpl2.deleted_at IS NULL
...
-- Resumen
SELECT 'Lote 14 migrado: LPL + seguimiento_diario' AS paso;
...
SELECT 'Registros en seguimiento_diario (levante, lote 14)' AS paso, COUNT(*) AS total
FROM public.seguimiento_diario sd
WHERE sd.tipo_seguimiento = 'levante'
  AND sd.lote_postura_levante_id = (SELECT lote_postura_levante_id FROM public.lote_postura_levante WHERE lote_id = 14 AND deleted_at IS NULL LIMIT 1);

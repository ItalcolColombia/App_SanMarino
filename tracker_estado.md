...
- [ ] Front: `aves-engorde` vs `aves-engorde-panama` → funciones/modelos compartidos, orquestadores por país
  ...
  - [ ] DECISIÓN USUARIO (reformulada): los 6 insumos de liquidación Panamá viven en el modal del módulo compartido (gated `esPanama`), pero el módulo fork `aves-engorde-panama` tiene un modal viejo SIN ellos. ¿Por cuál ruta liquidan hoy los usuarios de Panamá: `/daily-log/aves-engorde` (verían los insumos) o `/daily-log/aves-engorde-panama` (no los ven)? Según la respuesta: portar insumos al fork o retirarlos del compartido.
  - [ ] BUG anotado: "Cuadrar saldos" del módulo Panamá también da 404 y su backend no existe para la tabla `_panama` — decidir si se implementa o se oculta el botón en ese módulo.

...
- [ ] Front: `aves-engorde` vs `aves-engorde-panama` → funciones/modelos compartidos, orquestadores por país
  ...
  - [x] **RESUELTO (usuario aprobó eliminar)**: fork `aves-engorde-panama` eliminado por completo — ciclo 23 `6b5b6cd`: módulo front (30 archivos) + ruta lazy + controller + service + interfaz + DI + entidad/config/DbSet. Migración `RemoveSeguimientoDiarioAvesEngordePanamaFromModel` (Up vacío: solo snapshot; tabla física → candidata a DROP en `propuesta_drop_tablas_sin_uso.sql`). Validado: build 0/0, 54 tests, ng build OK, smoke E2E Panamá sobre módulo compartido.
...
- **C2 EN CURSO** (producción postura): diagnóstico hecho. Tabla viva YA server-only; **gráficas: fallback cliente eliminado** (commit `bb1bf2c`, validado E2E P-K345A: 43 filas, 8 canvas, 0 NG0103). Hallazgo bonus: trío de componentes duplicados MUERTOS (`pages/tabla-lista-indicadores`, `components/graficas-principal`, `components/tabs-principal`) → candidatos a borrar.
  ...
  - [x] **Backend migrado a SQL** (agente dev en paralelo, revisado e integrado): `IndicadoresProduccionService` 727→~250 líneas, delega en `fn_indicadores_produccion_postura` vía `SqlQueryRaw` (mismo contrato `IndicadoresProduccionResponse`). Migración a mano idempotente `20260703120000_AddFnIndicadoresProduccionPostura` (+Designer; ModelSnapshot intacto; `has-pending-model-changes`=No changes). Cálculo puro en `IndicadoresProduccionCalculos.cs`. **Equivalencia: 0 diferencias** (62 campos × 43 sem × P-K345A + P-K345B, epsilon 1e-6: C# original ≡ fn ≡ servicio). **E2E**: producción 43 filas, 0 errores. Política replicar+corregir bugs guía (REQ-004, ya en el C#). `dotnet build` 0/0 · `dotnet test` 70. Spec en `fase_de_desarrollo/c2_indicadores_produccion_a_sql.md`.
  - **C2 CERRADO.** Reportes → SQL (siguiente): plan en `fase_de_desarrollo/reportes_a_sql_plan.md` (P1 ReporteTecnicoProduccion, P2 ReporteTecnico levante, P3 ReporteContable).
- ⚠️ **HALLAZGO REPO**: `.gitignore` línea 20 (`/backend/tests`) ignora TODO `backend/tests/` → el suite real (63+ tests) vive solo en disco local, NUNCA commiteado (solo `UnitTest1.cs`+`.csproj` trackeados). Los tests C2 se force-añadieron. Decisión pendiente: ¿versionar el suite completo?

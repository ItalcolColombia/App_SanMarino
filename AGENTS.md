# AGENTS.md — Guía operativa del repositorio

Guía **vinculante** para Codex en este monorepo: backend **.NET 10 LTS (Clean Architecture)** + frontend **Angular 22 (standalone) + TypeScript 6**, desplegado en **AWS ECS**. Estas reglas **anulan** cualquier comportamiento por defecto.
...
> ⚠️ **Sesiones en paralelo — NUNCA pises el tracker de otra sesión.** El usuario suele tener **varias ventanas de Codex trabajando el mismo repo a la vez**. Antes de escribir el tracker, mirá lo que ya está:
...
4. **Si hay que convivir con un backend ajeno** (otra ventana de Codex, ver §⚙️), no lo mates a ciegas: redirigí TU salida de build para no pelear por el `bin/` — `dotnet build --artifacts-path <dir>` (o las variables `UseArtifactsOutput=true` + `ArtifactsPath`, que además sirven con `dotnet-ef migrations add`), y corré el dll con `PORT=5501` + `--contentRoot <dir del API>`. `Program.cs` toma el puerto de la config `PORT`: `ASPNETCORE_URLS` no hace nada.

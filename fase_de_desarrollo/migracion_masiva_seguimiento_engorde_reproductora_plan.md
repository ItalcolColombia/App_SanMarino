...
## Ampliación 3 (pedida durante el desarrollo) — selector de lote elegible + reproductora en pantalla

1. **Fix "0 lotes" del paso 2:** el selector de lote del filtro jerárquico genérico lista los lotes de la tabla base (postura) — para los tipos de engorde mostraba 0. La página ahora usa `GET /api/Migracion/elegibles` (fuente correcta por tipo: engorde = `lote_ave_engorde` abiertos; reproductora = con ≥1 lote reproductora), refrescado por la cascada granja/núcleo/galpón (`showLoteSelection=false` en el filtro).
2. **Selector opcional de reproductora:** endpoint nuevo `GET /api/Migracion/reproductoras?loteId=` (id, nombre, código, encaset, cargados/confirmados) + `MigracionContextoDto.ReproductoraId`. Con reproductora elegida, la columna "Reproductora" del Excel puede quedar vacía (todas las filas cargan a esa); una fila que nombre OTRA reproductora del mismo lote es error; sin selección, la columna es obligatoria por fila (comportamiento anterior). Default del select: "Todas — el Excel indica la reproductora de cada fila".
3. **Reproductora multi-lote por nombres:** el esquema reproductora también recibe `Granja/Núcleo/Galpón/Lote` opcionales (mismos helpers case/acento-insensibles que engorde) — un archivo puede cargar reproductoras de varios lotes; "Reproductora" pasa a opcional a nivel ENCABEZADO (la obligatoriedad por celda vive en el parser). Plantilla con Referencias (reproductoras del lote + lotes abiertos ubicados) y dropdowns.

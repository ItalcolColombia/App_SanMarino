...
| `fn_inventario_gastos_existencias` hace `LEFT JOIN` asumiendo **una** fila de stock por (granja, ítem); con N silos multiplica filas | 🟠 alta | El módulo Gastos **no está habilitado en SR** (`/inventario-gastos` no está en sus `company_menus`) ⇒ no rompe hoy. Se deja `SUM` + `GROUP BY` en la fn como trabajo de la Fase D **antes** de habilitarlo. Documentado para que no sorprenda |

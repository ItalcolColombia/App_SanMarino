 ...
 * Este cálculo espeja el backend `IndicadorEngordeService` — NO alterar la matemática.
 */
import {
  IndicadorEngordeDto,
...
export function ajusteAvesDe(ind: IndicadorEngordeDto): number {
...
export function porcentajeAjusteAvesDe(ind: IndicadorEngordeDto): number {
...
export function calcularLiquidacionTotales(
  ...
  }, null as IndicadorEngordeDto | null) ?? first;
  ...
  const w = (fn: (x: IndicadorEngordeDto) => number) =>

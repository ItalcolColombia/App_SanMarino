// frontend/src/app/features/indicador-engorde/pages/indicador-engorde-list/indicador-engorde-list.component.ts
...
import {
  IndicadorEngordeService,
  IndicadorEngordeDto,
  IndicadorEngordeRequest,
  IndicadorEngordeConsolidadoDto,
...
} from '../../services/indicador-engorde.service';
...
@Component({
  selector: 'app-indicador-engorde-list',
  standalone: true,
  ...
  templateUrl: './indicador-engorde-list.component.html',
  changeDetection: ChangeDetectionStrategy.Eager,
  styleUrls: ['./indicador-engorde-list.component.scss']
})
export class IndicadorEngordeListComponent implements OnInit {
  ...
  consolidado: IndicadorEngordeConsolidadoDto | null = null;
  mostrarConsolidado: boolean = false;
  lotesCerrados: IndicadorEngordeDto[] = [];
  ...
  // Inyección moderna
  private indicadorService = inject(IndicadorEngordeService);
  ...
  async calcularIndicadores(): Promise<void> {
    ...
    try {
      const request: IndicadorEngordeRequest = {
  ...
  liquidacionTotales(): IndicadorEngordeDto | null {
   ...
   */
  ajusteDe(ind: IndicadorEngordeDto): number {
    return ajusteAvesDe(ind);
  }

  /** % de ajuste = (ajuste / encasetadas) × 100. Se calcula para todos los lotes. */
  porcentajeAjusteDe(ind: IndicadorEngordeDto): number {

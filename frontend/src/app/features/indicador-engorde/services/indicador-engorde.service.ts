// frontend/src/app/features/indicador-engorde/services/indicador-engorde.service.ts
...
export interface IndicadorEngordeRequest {
...
export interface IndicadorEngordeDto {
  granjaId: number;
  granjaNombre: string;
  loteId: number | null;
...
export interface IndicadorEngordeConsolidadoDto {
  fechaCalculo: string;
  totalGranjas: number;
  totalLotes: number;
  ...
  indicadoresPorGranja: IndicadorEngordeDto[];
}
...
export interface LiquidacionPeriodoDto {
  ...
  totalGranjas: number;
  totalLotesCerrados: number;
  indicadores: IndicadorEngordeDto[];
}
...
export interface IndicadorPolloEngordePorLotePadreDto {
  indicadorLotePadre: IndicadorEngordeDto | null;
...
export class IndicadorEngordeService {
  private readonly baseUrl = `${environment.apiUrl}/IndicadorEngorde`;
  ...
  calcularIndicadores(request: IndicadorEngordeRequest): Observable<IndicadorEngordeDto[]> {
    return this.http.post<IndicadorEngordeDto[]>(`${this.baseUrl}/calcular`, request);
  }

  calcularConsolidado(request: IndicadorEngordeRequest): Observable<IndicadorEngordeConsolidadoDto> {
    return this.http.post<IndicadorEngordeConsolidadoDto>(`${this.baseUrl}/consolidado`, request);
  }
  ...
  obtenerLotesCerrados(
  ...
  ): Observable<IndicadorEngordeDto[]> {
    let params = new HttpParams()
    ...
    return this.http.get<IndicadorEngordeDto[]>(`${this.baseUrl}/lotes-cerrados`, { params });
  }

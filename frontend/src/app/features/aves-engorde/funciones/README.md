...
| `separar-stock-por-ubicacion.funcion.ts` | Separa las filas de stock de alimento en las que son **del lote** (su galpón, o stock de nivel núcleo/granja) y las de **otro galpón**. La usa el modal de liquidación: cuando el galpón no tiene alimento vuelve a consultar sin filtrar por galpón —para no perder el stock de nivel núcleo/granja— y esa consulta trae también a los vecinos. Contarlos hacía que 15 lotes avisaran «hay alimento en inventario» con kilos ajenos (medido 17ago26). |
| `validar-insumos-panama.funcion.ts` | Rechaza decimales en los 4 insumos **enteros** de la liquidación Panamá (`diasEnGranja`, `diasEngorde`, `avesFinalGranja`, `avesBeneficiada`) ANTES de enviarlos: el backend los tipa `int` y un decimal ahí rompía la deserialización del JSON con un 400 sin mensaje utilizable (lote 13-1, 26-ago-2026). |
| `extraer-mensaje-error.funcion.ts` | Lee el mensaje real de un `HttpErrorResponse` — `err.message` de Angular es siempre el genérico ("Http failure response for URL: 400 OK"), nunca el motivo. Cubre las 3 formas que devuelve el backend: `{error}`/`{message}` (excepciones de negocio) y `{title, errors}` (`ValidationProblemDetails` automático de `[ApiController]` cuando el JSON no calza con el contrato). |
